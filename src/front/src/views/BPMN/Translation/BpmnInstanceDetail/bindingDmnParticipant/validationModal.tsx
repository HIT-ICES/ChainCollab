...
const getXmlLocalName = (element?: Element | null) =>
  (element?.localName || element?.tagName || "")
    .split(":")
    .pop()
    ?.toLowerCase() || "";

const splitXmlRefs = (value?: string | null): string[] =>
  (value || "")
    .split(/\s+/)
    .map((ref) => ref.trim())
    .filter(Boolean);
...
const getDirectChildText = (element: Element, localName: string) => {
  const normalizedName = localName.toLowerCase();
  for (let i = 0; i < element.children.length; i++) {
    const child = element.children[i];
    if (getXmlLocalName(child) === normalizedName) {
...
const findFirstDocumentation = (element: Element) => {
  const docs = Array.from(element.getElementsByTagName("*")).filter(
    (child) => getXmlLocalName(child) === "documentation",
  );
  return docs[0]?.textContent || "";
};

const findParentTask = (element: Element | null) => {
  let taskElement = element?.parentElement || null;
  let depth = 0;
  while (
    taskElement &&
    !getXmlLocalName(taskElement).includes("task") &&
    depth < 10
  ) {
...
const readAssetOperation = (taskElement: Element) => {
  const operationElement = Array.from(
    taskElement.getElementsByTagName("*"),
  ).find((child) => getXmlLocalName(child) === "assetoperation");

  if (!operationElement) return null;

  const outputs: Record<string, { type: string; dataType: string }> = {};
  Array.from(operationElement.children).forEach((child) => {
    if (getXmlLocalName(child) !== "output") return;
  ...
  return {
    operation: operationElement.getAttribute("operation") || "",
    inputAssetRefs: splitXmlRefs(
      operationElement.getAttribute("inputAssetRefs"),
    ),
    outputAssetRefs: splitXmlRefs(
      operationElement.getAttribute("outputAssetRefs"),
    ),
    recipientRefs: splitXmlRefs(operationElement.getAttribute("recipientRefs")),
    tokenNumber: operationElement.getAttribute("tokenNumber") || "",
    outputs,
  };
};
...
const sortTaskInfosByProcessFlow = (xmlDoc: Document, taskInfos: any[]) => {
  const relatedTaskIds = Array.from(
    new Set(taskInfos.map((taskInfo: any) => taskInfo.taskId).filter(Boolean)),
  );
  const allElements = xmlDoc.getElementsByTagName("*");
  const adjacencyList: Map<string, string[]> = new Map();
  const relatedIndex = new Map(
    relatedTaskIds.map((taskId, index) => [taskId, index]),
  );

  for (let i = 0; i < allElements.length; i++) {
    const el = allElements[i];
    if (getXmlLocalName(el) !== "sequenceflow") continue;

    const sourceRef = el.getAttribute("sourceRef");
    const targetRef = el.getAttribute("targetRef");
    if (!sourceRef || !targetRef) continue;

    if (!adjacencyList.has(sourceRef)) {
      adjacencyList.set(sourceRef, []);
    }
    adjacencyList.get(sourceRef)?.push(targetRef);
  }

  const hasPath = (sourceId: string, targetId: string) => {
    const queue = [...(adjacencyList.get(sourceId) || [])];
    ...
    while (queue.length > 0) {
      ...
      visited.add(current);
      queue.push(...(adjacencyList.get(current) || []));
    }
  ...
  const relatedAdjacencyList: Map<string, string[]> = new Map();

  relatedTaskIds.forEach((taskId) => {
    inDegree.set(taskId, 0);
    relatedAdjacencyList.set(taskId, []);
  });

  relatedTaskIds.forEach((sourceId) => {
    relatedTaskIds.forEach((targetId) => {
      if (sourceId === targetId) return;
      const sourceReachesTarget = hasPath(sourceId, targetId);
      const targetReachesSource = hasPath(targetId, sourceId);
      if (!sourceReachesTarget || targetReachesSource) return;

      relatedAdjacencyList.get(sourceId)?.push(targetId);
      inDegree.set(targetId, (inDegree.get(targetId) || 0) + 1);
    });
  });

  const sortedTasks: string[] = [];
  const queue = relatedTaskIds
    .filter((taskId) => inDegree.get(taskId) === 0)
    .sort((a, b) => (relatedIndex.get(a) || 0) - (relatedIndex.get(b) || 0));

  while (queue.length > 0) {
    const current = queue.shift()!;
    if (sortedTasks.includes(current)) continue;

    sortedTasks.push(current);
    taskOrder.set(current, sortedTasks.length - 1);

    const neighbors = relatedAdjacencyList.get(current) || [];
    neighbors.forEach((neighbor) => {
      ...
      if (newDegree === 0) {
        queue.push(neighbor);
        queue.sort(
          (a, b) => (relatedIndex.get(a) || 0) - (relatedIndex.get(b) || 0),
        );
      }
    });
  }
...
export const ValidationModal: React.FC<ValidationModalProps> = ({
  ...
  const [chaincodeToNameMap, setChaincodeToNameMap] = useState<
  ...
  const extractParticipantInfo = (paramData: any): ParticipantInfo[] => {
    ...
    for (const key of Object.keys(paramData)) {
      ...
      const x509Encoded =
        atIndex > -1 ? x509Full.substring(0, atIndex) : x509Full;
  ...
  const invokeContractMethod = async (
    ...
    try {
      ...
      console.error(
        `[invokeContractMethod] 调用合约方法 ${methodName} 失败:`,
        error,
      );
      message.error(
        `调用合约方法 ${methodName} 失败: ${error?.message || "未知错误"}`,
      );
      throw error;
    }
  };
  ...
  const FTqueryBalanceOf = async (
    ...
    for (const [, apiName] of Object.entries(chaincodeToNameMap)) {
      ...
      if (apiName.startsWith(chaincodeName)) {
        ...
        console.log(
          `[FTqueryBalanceOf] 匹配成功: ${chaincodeName} -> ${apiName}`,
        );
        break;
      }
    }
  ...
  const NFTqueryOwnerOf = async (chaincodeName: string, tokenId: string) => {
    ...
    for (const [, apiName] of Object.entries(chaincodeToNameMap)) {
      ...
      if (apiName.startsWith(chaincodeName)) {
        matchedApiName = apiName;
        console.log(
          `[NFTqueryOwnerOf] 匹配成功: ${chaincodeName} -> ${apiName}`,
        );
        break;
      }
    }
  ...
  const getFireflyApiList = async (fireflyBaseUrl: string) => {
    try {
      ...
      console.log(
        `[getFireflyApiList] 获取到 ${Array.isArray(apisList) ? apisList.length : 0} 个 API`,
      );
  ...
  const performAssetTypeBasedValidation = async () => {
    try {
      ...
      const allElements = xmlDoc.getElementsByTagName("*");
      const dataObjects = Array.from(allElements).filter(
        (el) => getXmlLocalName(el) === "dataobjectreference",
      );
      const assetObjects = Array.from(allElements).filter(
        (el) => getXmlLocalName(el) === "asset",
      );

      // 存储每个 DataObject 及其关联的 Tasks
      const dataObjectTaskMap: Record<
        string,
        {
          dataObjectId: string;
          assetType: string;
      ...
      const appendTaskRef = (
        ...
        if (!dataObjectTaskMap[dataObjectId] || !taskRef.taskId) return;
        const exists = dataObjectTaskMap[dataObjectId].tasks.some(
          (task) =>
            task.taskId === taskRef.taskId && task.isInput === taskRef.isInput,
        );
        if (!exists) {
          dataObjectTaskMap[dataObjectId].tasks.push(taskRef);
        }
      };
      ...
      dataObjects.forEach((dataObj: any) => {
        ...
        const docText = findFirstDocumentation(dataObj);
        if (docText) {
          try {
            ...
            dataObjectTaskMap[dataObjectId] = {
              ...
              tokenURL: assetInfo.tokenURL,
              tokenHasExistInERC: assetInfo.tokenHasExistInERC || false,
              tasks: [],
            };
          } catch (error) {
            console.warn(
              `Failed to parse documentation for ${dataObjectId}:`,
              error,
            );
          }
        }
      ...
      // 新 Asset 建模：资产信息存在 abc:Asset 属性上。
      assetObjects.forEach((assetObj: any) => {
        const dataObjectId = assetObj.getAttribute("id");
        if (!dataObjectId) return;

        dataObjectTaskMap[dataObjectId] = {
          dataObjectId,
          assetType: assetObj.getAttribute("assetType") || "",
          tokenType: assetObj.getAttribute("tokenType") || undefined,
          tokenName:
            assetObj.getAttribute("tokenName") ||
            assetObj.getAttribute("name") ||
            undefined,
          tokenId: assetObj.getAttribute("tokenId") || undefined,
          tokenURL: assetObj.getAttribute("tokenURL") || undefined,
          tokenHasExistInERC: parseXmlBoolean(
            assetObj.getAttribute("tokenHasExistInERC"),
          ),
          tasks: dataObjectTaskMap[dataObjectId]?.tasks || [],
        };
      });

      for (let i = 0; i < allElements.length; i++) {
        const el = allElements[i];
        const tagName = getXmlLocalName(el);

        // 处理 DataInputAssociation (DataObject -> Task)
        if (tagName === "datainputassociation") {
          const sourceRef = getDirectChildText(el, "sourceRef");

          // 找到父 Task
          const taskElement = findParentTask(el);

          if (taskElement && sourceRef && dataObjectTaskMap[sourceRef]) {
            const taskId = taskElement.getAttribute("id");
            const taskDoc = findFirstDocumentation(taskElement);
            let operation = "";
            let documentation = "";

            if (taskDoc) {
              try {
                documentation = taskDoc;
                const taskInfo = JSON.parse(documentation);
                operation = taskInfo.operation || "";
              } catch {}
            }

            appendTaskRef(sourceRef, {
              taskId,
              operation,
              documentation,
        ...
        if (tagName === "dataoutputassociation") {
          const targetRef = getDirectChildText(el, "targetRef");
        ...
        // 新 AssetTask 建模：Task 与 Asset 的关系在 abc:AssetOperation 的 input/output refs 中。
        if (tagName === "choreographytask") {
          const taskId = el.getAttribute("id") || "";
          const assetOperation = readAssetOperation(el);
          if (!taskId || !assetOperation) continue;

          const createOps = ["mint", "branch", "merge"];
          const primaryAssetRef = createOps.includes(assetOperation.operation)
            ? assetOperation.outputAssetRefs[0] ||
              assetOperation.inputAssetRefs[0]
            : assetOperation.inputAssetRefs[0] ||
              assetOperation.outputAssetRefs[0];
          const assetInfo = primaryAssetRef
            ? dataObjectTaskMap[primaryAssetRef]
            : undefined;
          const refTokenIds = assetOperation.inputAssetRefs
            .map((assetRef) => dataObjectTaskMap[assetRef]?.tokenId)
            .filter(Boolean);
          const docObj: Record<string, any> = {
            assetType: assetInfo?.assetType,
            operation: assetOperation.operation,
            tokenName: assetInfo?.tokenName,
            caller: el.getAttribute("initiatingParticipantRef") || "",
            callee: deriveCalleeRefs(
              el,
            ...
            tokenId: assetInfo?.tokenId,
            tokenURL: assetInfo?.tokenURL,
            tokenNumber: assetOperation.tokenNumber,
            tokenHasExistInERC: assetInfo?.tokenHasExistInERC || false,
            outputs: assetOperation.outputs,
          };
          if (refTokenIds.length > 0) {
            docObj.refTokenIds = refTokenIds;
          }
          const documentation = JSON.stringify(docObj);

          assetOperation.inputAssetRefs.forEach((assetRef) => {
            appendTaskRef(assetRef, {
              taskId,
              operation: assetOperation.operation,
              documentation,
              isInput: false,
          ...
          assetOperation.outputAssetRefs.forEach((assetRef) => {
            appendTaskRef(assetRef, {
              taskId,
              operation: assetOperation.operation,
              documentation,
      ...
      for (const [dataObjectId, info] of Object.entries(dataObjectTaskMap)) {
        ...
        // 分支 4: Distributive NFT
          ...
          console.warn(
            `Unknown asset type combination: ${info.assetType} / ${info.tokenType}`,
          );
          result = {
            dataObjectId: info.dataObjectId,
            assetType: info.assetType,
      ...
      if (failedResults.length > 0) {
        ...
        Modal.error({
          ...
          content: (
            <div style={{ maxHeight: "400px", overflow: "auto" }}>
              <p>
                共验证 {allValidationResults.length} 个 DataObject，其中{" "}
                {failedResults.length} 个失败：
              </p>
              {failedResults.map((result, idx) => (
                ...
                >
                  <p style={{ fontWeight: "bold", color: "#ff4d4f" }}>
                    {result.dataObjectId} ({result.assetType}
                    {result.tokenType ? ` - ${result.tokenType}` : ""})
                  </p>
                  <ul style={{ paddingLeft: "20px", margin: "5px 0" }}>
                    {result.errors.map((err, errIdx) => (
                      <li key={errIdx} style={{ marginBottom: "3px" }}>
                        {err}
                      </li>
                    ))}
                  </ul>
              ...
              {successResults.length > 0 && (
                <div style={{ marginTop: "15px" }}>
                  <p style={{ color: "#52c41a" }}>
                    验证通过的 DataObject ({successResults.length} 个):
                  </p>
                  <ul style={{ paddingLeft: "20px", margin: "5px 0" }}>
        ...
        Modal.success({
          ...
          content: (
            <div style={{ maxHeight: "400px", overflow: "auto" }}>
              <p>
                共验证 {allValidationResults.length} 个 DataObject，全部通过：
              </p>
              {allValidationResults.map((result, idx) => (
                ...
                >
                  <p
                    style={{ fontWeight: "bold", color: "#52c41a", margin: 0 }}
                  >
                    {result.dataObjectId} ({result.assetType}
                    {result.tokenType ? ` - ${result.tokenType}` : ""})
                  </p>
                  ...
                  {result.finalUsageRights &&
                    result.finalUsageRights.length > 0 && (
                      <p style={{ margin: "5px 0 0 0" }}>
                        使用权: [{result.finalUsageRights.join(", ")}]
                      </p>
                    )}
                </div>
              ))}
   ...
   * - query: 不影响余额
   */
  const validateTransferableFT = async (
    info: any,
    ...
    const sortedTaskInfos = sortTaskInfosByProcessFlow(xmlDoc, info.tasks);
    ...
    const participantIds = Object.keys(param).filter(
      (key) => !specialKeys.includes(key),
    );

    // 构建 name 到 ID 的映射（用于将 caller/callee 中的 name 转换为 ID）
    const participantElements = xmlDoc.querySelectorAll(
      "participant, bpmn\\:participant, bpmn2\\:participant",
    );
    ...
    for (const task of info.tasks) {
      if (task.taskId && ercChaincodeNames[task.taskId]) {
        ...
        console.log(
          `[validateTransferableFT] 从 Task ${task.taskId} 获取到 ERC 合约名: ${ercContractName}`,
        );
        break;
      }
    }
    ...
    if (!ercContractName) {
      console.warn(
        "[validateTransferableFT] 未找到与此 DataObject 关联的 ERC 合约名，将使用默认余额 0",
      );
    }
    ...
    console.log(
      "[validateTransferableFT] participantIdToX509Map:",
      Object.fromEntries(participantIdToX509Map),
    );
    ...
    for (const participantId of participantIds) {
      if (!allParticipants.includes(participantId)) {
        ...
        if (ercContractName && x509Encoded) {
          ...
          try {
            console.log(
              `[validateTransferableFT] 查询 ${participantId} 的初始余额...`,
            );
            ...
            console.log(
              `[validateTransferableFT] ${participantId} 初始余额: ${balance}`,
            );
            setBalance(participantId, balance);
          } catch (error) {
            // 查询失败，默认余额为 0
            console.warn(
              `[validateTransferableFT] 查询 ${participantId} 余额失败，使用默认值 0:`,
              error,
            );
          ...
          console.log(
            `[validateTransferableFT] ${participantId} 无法查询余额（缺少合约名或 x509），使用默认值 0`,
          );
          setBalance(participantId, 0);
        }
      }
      ...
      if (operationLower === "mint") {
        ...
        if (tokenNumber <= 0) {
          ...
          if (!receiver) {
            ...
            console.log(
              `  ✓ mint 成功，${receiver} 余额: ${currentBalance} + ${tokenNumber} = ${newBalance}`,
            );
          }
        }
        ...
        if (tokenNumber <= 0) {
          ...
          if (senderBalance < tokenNumber) {
            ...
            console.log(
              `    ${caller}: ${senderBalance} - ${tokenNumber} = ${newSenderBalance}`,
            );
            console.log(
              `    ${callee}: ${receiverBalance} + ${tokenNumber} = ${newReceiverBalance}`,
            );
          }
        }
        ...
        if (tokenNumber <= 0) {
          ...
          if (burnerBalance < tokenNumber) {
            ...
            console.log(
              `  ✓ burn 成功，${caller} 余额: ${burnerBalance} - ${tokenNumber} = ${newBalance}`,
            );
          }
        }
   ...
   */
  const validateTransferableNFT = async (
    info: any,
  ): Promise<ValidationResult> => {
    ...
    if (tokenHasExistInERC) {
      ...
      if (!tokenId) {
        console.warn(
          "[validateTransferableNFT] tokenHasExistInERC=true 但未找到 tokenId，将使用默认所有者 none",
        );
      } else {
        ...
        for (const task of info.tasks) {
          if (task.taskId && ercChaincodeNames[task.taskId]) {
            ercContractName = ercChaincodeNames[task.taskId];
            console.log(
              `[validateTransferableNFT] 从 Task ${task.taskId} 获取到 ERC 合约名: ${ercContractName}`,
            );
            break;
          }
        }
        ...
        if (!ercContractName) {
          console.warn(
            "[validateTransferableNFT] 未找到与此 DataObject 关联的 ERC 合约名，将使用默认所有者 none",
          );
        } else {
          // 3. 调用 NFTqueryOwnerOf 查询当前所有者
          try {
            console.log(
              `[validateTransferableNFT] 查询 NFT (tokenId: ${tokenId}) 的当前所有者...`,
            );
            ...
            // 打印完整的返回结果以便调试
            console.log(
              `[validateTransferableNFT] NFTqueryOwnerOf 返回结果:`,
              result,
            );
            console.log(
              `[validateTransferableNFT] 返回结果类型:`,
              typeof result,
            );
            ...
            console.log(
              `[validateTransferableNFT] 解析后的所有者 x509: ${ownerX509}`,
            );
            ...
            if (ownerX509) {
              ...
              for (const pInfo of participantInfoList) {
                ...
                if (
                  ownerX509.includes(pInfo.x509Decoded) ||
                  pInfo.x509Decoded.includes(ownerX509)
                ) {
                  matchedParticipantId = pInfo.participantId;
                  console.log(
                    `[validateTransferableNFT] 匹配成功: ${ownerX509} -> ${pInfo.participantId}`,
                  );
                  break;
                }
              }
              ...
              if (matchedParticipantId) {
                ...
                console.log(
                  `[validateTransferableNFT] 初始所有者设置为: ${currentOwner}`,
                );
              } else {
                console.warn(
                  `[validateTransferableNFT] 未找到匹配的参与者，所有者设置为 none`,
                );
                currentOwner = "none";
              }
            } else {
              console.warn(
                `[validateTransferableNFT] 查询结果为空，所有者设置为 none`,
              );
              currentOwner = "none";
            }
          } catch (error) {
            console.warn(
              `[validateTransferableNFT] 查询 NFT 所有者失败，使用默认所有者 none:`,
              error,
            );
            currentOwner = "none";
          }
    ...
    for (const taskInfo of sortedTaskInfos) {
      ...
      console.log(
        `  当前所有者: "${currentOwner}" (length: ${currentOwner.length})`,
      );
      ...
      if (operationLower === "mint") {
        ...
        if (currentOwner === "none") {
          ...
          console.log(
            `  ✓ transfer 成功，所有者从 ${previousOwner} 变更为: ${currentOwner}`,
          );
        }
      } else if (operationLower === "burn") {
   ...
   */
  const validateValueAddedNFT = async (
    info: any,
  ): Promise<ValidationResult> => {
    ...
    if (!tokenHasExistInERC) {
      console.log(
        "[validateValueAddedNFT] tokenHasExistInERC=false，检查是否存在 merge 或 branch 创建操作...",
      );
      ...
      if (!hasCreationOperation) {
        console.error(
          `[validateValueAddedNFT] DataObject ${info.dataObjectId} 的 tokenHasExistInERC=false，但未找到 merge 或 branch 创建操作`,
        );
        return {
          dataObjectId: info.dataObjectId,
          assetType: info.assetType,
          ...
          success: false,
          errors: [
            `DataObject ${info.dataObjectId} 必须有至少一个 merge 或 branch 操作的 Task 指向它以创建 token`,
          ],
        };
      }
    ...
    if (tokenHasExistInERC) {
      ...
      if (!tokenId) {
        console.warn(
          "[validateValueAddedNFT] tokenHasExistInERC=true 但未找到 tokenId，将使用默认所有者 none",
        );
      } else {
        // 2. 获取 ERC 合约名（参考 FT 的获取逻辑）
        let ercContractName = "";
        ...
        for (const task of info.tasks) {
          if (task.taskId && ercChaincodeNames[task.taskId]) {
            ercContractName = ercChaincodeNames[task.taskId];
            console.log(
              `[validateValueAddedNFT] 从 Task ${task.taskId} 获取到 ERC 合约名: ${ercContractName}`,
            );
            break;
          }
        }
        ...
        if (!ercContractName) {
          console.warn(
            "[validateValueAddedNFT] 未找到与此 DataObject 关联的 ERC 合约名，将使用默认所有者 none",
          );
        } else {
          // 3. 调用 NFTqueryOwnerOf 查询当前所有者
          try {
            console.log(
              `[validateValueAddedNFT] 查询 NFT (tokenId: ${tokenId}) 的当前所有者...`,
            );
            const result = await NFTqueryOwnerOf(ercContractName, tokenId);

            // 解析返回结果，获取所有者信息
            console.log(
              `[validateValueAddedNFT] NFTqueryOwnerOf 返回结果:`,
              result,
            );
            ...
            console.log(
              `[validateValueAddedNFT] 解析后的所有者 x509: ${ownerX509}`,
            );
            ...
            if (ownerX509) {
              ...
              for (const pInfo of participantInfoList) {
                if (
                  ...
                  matchedParticipantId = pInfo.participantId;
                  console.log(
                    `[validateValueAddedNFT] 匹配成功: ${ownerX509} -> ${pInfo.participantId}`,
                  );
                  break;
                }
              }
              ...
              if (matchedParticipantId) {
                currentOwner = matchedParticipantId;
                console.log(
                  `[validateValueAddedNFT] 初始所有者设置为: ${currentOwner}`,
                );
              } else {
                console.warn(
                  `[validateValueAddedNFT] 未找到匹配的参与者，所有者设置为 none`,
                );
                currentOwner = "none";
              }
            } else {
              console.warn(
                `[validateValueAddedNFT] 查询结果为空，所有者设置为 none`,
              );
              currentOwner = "none";
            }
          } catch (error) {
            console.warn(
              `[validateValueAddedNFT] 查询 NFT 所有者失败，使用默认所有者 none:`,
              error,
            );
            currentOwner = "none";
          }
   ...
   */
  const validateDistributiveNFT = async (
    info: any,
  ): Promise<ValidationResult> => {
    ...
    if (tokenHasExistInERC) {
      ...
      if (!baseTokenId) {
        console.warn(
          "[validateDistributiveNFT] tokenHasExistInERC=true 但未找到 tokenId，将使用默认所有者 none",
        );
      } else {
        const tokenIdWithOwner = `${baseTokenId}_owner`;
        console.log(
          `[validateDistributiveNFT] 使用 tokenId: ${tokenIdWithOwner}`,
        );
        ...
        for (const task of info.tasks) {
          if (task.taskId && ercChaincodeNames[task.taskId]) {
            ercContractName = ercChaincodeNames[task.taskId];
            console.log(
              `[validateDistributiveNFT] 从 Task ${task.taskId} 获取到 ERC 合约名: ${ercContractName}`,
            );
            break;
          }
        }
        ...
        if (!ercContractName) {
          console.warn(
            "[validateDistributiveNFT] 未找到与此 DataObject 关联的 ERC 合约名，将使用默认所有者 none",
          );
        } else {
          ...
          for (const pInfo of participantInfoList) {
            try {
              console.log(
                `[validateDistributiveNFT] 查询参与者 ${pInfo.participantId} 的余额...`,
              );
              console.log(
                `[validateDistributiveNFT] account (x509Encoded): ${pInfo.x509Encoded.substring(0, 50)}...`,
              );

              const result = await DisNFTquery(
                ercContractName,
                tokenIdWithOwner,
                pInfo.x509Encoded,
              );
              ...
              // 解析返回结果
              console.log(
                `[validateDistributiveNFT] DisNFTquery 返回结果:`,
                result,
              );
              console.log(
                `[validateDistributiveNFT] 返回结果类型:`,
                typeof result,
              );
              ...
              if (balance === 1) {
                ...
                console.log(
                  `[validateDistributiveNFT] 找到所有者: ${currentOwner}`,
                );
                break;
              }
            } catch (error) {
              console.warn(
                `[validateDistributiveNFT] 查询参与者 ${pInfo.participantId} 余额失败:`,
                error,
              );
              // 继续查询下一个参与者
            }
          ...
          if (currentOwner === "none") {
            console.warn(
              `[validateDistributiveNFT] 未找到任何参与者拥有该 token，所有者设置为 none`,
            );
          } else {
            console.log(
              `[validateDistributiveNFT] 初始所有者设置为: ${currentOwner}`,
            );
          }
        }
      }
    ...
    for (const taskInfo of sortedTaskInfos) {
      ...
      if (operationLower === "mint") {
        ...
        if (currentOwner === "none") {
          ...
          console.log(
            `  ✓ burn 成功，所有者从 ${currentOwner} 变更为: none，使用权清空`,
          );
          currentOwner = "none";
          usageRights = [];
        }
        ...
        if (currentOwner === "none") {
          ...
          console.log(
            `  ✓ grant usage rights 成功，新增使用权: [${newGrantees.join(", ")}]`,
          );
        ...
        if (currentOwner === "none") {
          ...
          const notInUsageRights = calleeArray.filter(
            (grantee) => grantee && !usageRights.includes(grantee),
          );
          if (notInUsageRights.length > 0) {
            ...
            console.log(
              `  ✓ revoke usage rights 成功，移除使用权: [${revokedGrantees.join(", ")}]`,
            );
            console.log(`  当前使用权列表: [${usageRights.join(", ")}]`);
          }
        }
  ...
  const handleValidation = async (): Promise<boolean> => {
    ...
    try {
      ...
      console.log(
        "BPMN XML first 500 chars:",
        bpmnXml ? bpmnXml.substring(0, 500) : "NULL",
      );
      ...
      console.log(
        "Number of tasks with ERC bindings:",
        Object.keys(ercChaincodeNames).length,
      );

      // 构建 Asset/DataObject -> Tasks 映射
      const dataObjectToTasks: Record<string, Set<string>> = {};
      const addDataObjectTaskMapping = (
        dataObjectId: string,
        taskId: string,
      ) => {
        if (!dataObjectId || !taskId || !ercChaincodeNames[taskId]) return;
        if (!dataObjectToTasks[dataObjectId]) {
          dataObjectToTasks[dataObjectId] = new Set();
        }
        dataObjectToTasks[dataObjectId].add(taskId);
      };
      ...
      // 遍历所有元素，查找旧 DataAssociation 和新 AssetOperation refs
      for (let i = 0; i < allElements.length; i++) {
        const el = allElements[i];
        const tagName = getXmlLocalName(el);
        ...
        if (tagName === "datainputassociation") {
          ...
          if (taskElement) {
            ...
            console.log(
              `  Has ERC binding: ${taskId && ercChaincodeNames[taskId] ? "Yes" : "No"}`,
            );
            ...
            if (taskId && ercChaincodeNames[taskId]) {
              ...
              if (sourceRef) {
                console.log(
                  `  ✓ Mapping: DataObject ${sourceRef} -> Task ${taskId} (${ercChaincodeNames[taskId]})`,
                );
                addDataObjectTaskMapping(sourceRef, taskId);
              }
            }
          } else {
        ...
        if (tagName === "dataoutputassociation") {
          ...
          if (taskElement) {
            ...
            if (taskId && ercChaincodeNames[taskId]) {
              ...
              if (targetRef) {
                console.log(
                  `  ✓ Mapping: Task ${taskId} (${ercChaincodeNames[taskId]}) -> DataObject ${targetRef}`,
                );
                addDataObjectTaskMapping(targetRef, taskId);
              }
            }
          } else {
            console.log(`  ✗ No parent Task found`);
        ...
        if (tagName === "choreographytask") {
          const taskId = el.getAttribute("id") || "";
          const assetOperation = readAssetOperation(el);
          if (!taskId || !assetOperation || !ercChaincodeNames[taskId])
            continue;

          const assetRefs = Array.from(
            new Set([
              ...assetOperation.inputAssetRefs,
              ...assetOperation.outputAssetRefs,
            ]),
          );
          assetRefs.forEach((assetRef) => {
            console.log(
              `  ✓ Mapping: Asset ${assetRef} -> AssetTask ${taskId} (${ercChaincodeNames[taskId]})`,
            );
            addDataObjectTaskMapping(assetRef, taskId);
          });
        }
      }
      ...
      console.log(
        "Number of DataObjects found:",
        Object.keys(dataObjectToTasks).length,
      );
      ...
      for (const [dataObjectId, taskIds] of Object.entries(dataObjectToTasks)) {
        ...
        const contracts = taskIdArray
          .map((taskId) => ercChaincodeNames[taskId])
          .filter(Boolean);
      ...
      if (errors.length > 0) {
        ...
        message.error(
          "校验失败：存在 DataObject 连接的 Tasks 使用了不同的合约名",
        );
        return false;
      } else {
        setValidationResult({
    ...
    >
      ...
      {validationResult && (
        <div style={{ marginBottom: "20px" }}>
          ...
          {validationResult.success &&
            validationResult.dataObjectContractMap && (
              <div style={{ marginTop: "16px" }}>
                <h4>DataObject → 合约名映射：</h4>
                <Table
                  dataSource={Object.entries(
                    validationResult.dataObjectContractMap,
                  ).map(([dataObjectId, contractName], index) => ({
                    key: index,
                    dataObjectId,
                    contractName,
                  }))}
      ...
      <div
        style={{ padding: "10px", background: "#f0f0f0", borderRadius: "4px" }}
      >
        <p style={{ margin: 0, color: "#666" }}>
          <strong>说明：</strong>点击"执行校验"按钮将会校验 BPMN 图中的
          Asset/DataObject 与 Task 的合约绑定关系。 校验逻辑会检查连接到同一个
          Asset/DataObject 的所有 Tasks 是否使用了相同的 ERC 合约名。
          如果校验通过，将生成 Asset/DataObject → 合约名的映射关系供后续使用。
        </p>
      </div>
    </Modal>
  );
