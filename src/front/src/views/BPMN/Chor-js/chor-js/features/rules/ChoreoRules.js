...
ChoreoRules.prototype.init = function () {
  ...
  this.addRule('elements.move', function (context) {
    ...
    var target = context.target,
      ...
      position = context.position;
    return self.canAttach(shapes, target, null, position) || self.canReplace(shapes, target, position) || self.canMove(shapes, target, position) || self.canInsert(shapes, target, position);
  });
  this.addRule('shape.create', function (context) {
    // This function is equivalent to bpmn-js 6.5.1 except for self call
    return self.canCreate(context.shape, context.target, context.source, context.position);
  });
