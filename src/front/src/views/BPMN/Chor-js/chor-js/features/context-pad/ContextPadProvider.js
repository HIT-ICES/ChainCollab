...
ContextPadProvider.$inject = ['injector'];
...
ContextPadProvider.prototype.getContextPadEntries = function (element) {
  ...
  function appendAction(type, className, title, options) {
    if (typeof title !== 'string') {
      options = title;
      title = self._translate('Append {type}', {
        type: type.replace(/^bpmn:/, '')
      });
    }
    function appendStart(event, element) {
      var shape = self._elementFactory.createShape(assign({
        type: type
      }, options));
  ...
  if (is(businessObject, 'bpmn:FlowNode')) {
    ...
    if (!is(businessObject, 'bpmn:EndEvent')) {
      assign(actions, {
        ...
        'append.choreography-task': appendAction('bpmn:ChoreographyTask', 'choreo-icon-choreography-task')
      });
    }
    ...
    if (is(businessObject, 'bpmn:EventBasedGateway')) {
      assign(actions, {
        'append.timer-intermediate-event': appendAction('bpmn:IntermediateCatchEvent', 'bpmn-icon-intermediate-event-catch-timer', this._translate('Append TimerIntermediateCatchEvent'), {
    ...
    if (!is(businessObject, 'bpmn:EventBasedGateway') && !is(businessObject, 'bpmn:EndEvent')) {
      assign(actions, {
        ...
        'append.gateway-parallel': appendAction('bpmn:ParallelGateway', 'bpmn-icon-gateway-parallel', this._translate('Append ParallelGateway')),
        'append.gateway-event': appendAction('bpmn:EventBasedGateway', 'bpmn-icon-gateway-eventbased', this._translate('Append EventBasedGateway')),
        'append.gateway-xor': appendAction('bpmn:ExclusiveGateway', 'bpmn-icon-gateway-xor', this._translate('Append ExclusiveGateway'))
      });
    }
  ...
  if (is(element, 'bpmn:Participant')) {
    ...
    if (this._rules.allowed('band.canInitiatingBeSwapped', {
      bandShape: element
    })) {
      assign(actions, {
  ...
  let messageToggle = this._rules.allowed('message.toggle', {
    element: element
  });
  ...
  var deleteAllowed = this._rules.allowed('elements.delete', {
    elements: [element]
  });
