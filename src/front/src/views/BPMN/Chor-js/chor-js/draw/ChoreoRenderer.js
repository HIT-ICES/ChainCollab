...
export default function ChoreoRenderer(config, eventBus, textRenderer, pathMap) {
  ...
  function getFillColor(di, override) {
    return di.get('bioc:fill') || config && config.defaultFillColor || override || 'white';
  }
  ...
  this.drawMessage = function (p, element) {
    ...
    svgAttr(connector, {
      d: componentsToPath([['M', element.width / 2, isBottom ? -MESSAGE_DISTANCE : element.height], ['l', 0, MESSAGE_DISTANCE]]),
  ...
  function attachMarkerToChoreoActivity(parentGfx, element) {
    ...
    let hasLoopMarker = ['Standard', 'MultiInstanceSequential', 'MultiInstanceParallel'].indexOf(loopType) >= 0;
    let isCollapsed = element.collapsed;
    let offset = isCollapsed && hasLoopMarker ? 10 : 0;

    // Draw sub choreography marker
    if (isCollapsed) {
      translate(drawRect(parentGfx, 14, 14, assign({
        strokeWidth: 2
      }, style)), element.width / 2 - 7.5 + offset, element.height - bottomBandHeight - 20);
...
function getEnvelopePath(width, height) {
  let flap = height * 0.6;
  let path = [['M', 0, 0], ['l', 0, height], ['l', width, 0], ['l', 0, -height], ['z'], ['M', 0, 0], ['l', width / 2., flap], ['l', width / 2., -flap]];
  return componentsToPath(path);
}
function getMessageOutline(x, y, width, height) {
  let path = [['M', x, y], ['l', 0, height], ['l', width, 0], ['l', 0, -height], ['z']];
  return componentsToPath(path);
}
function getTaskOutline(x, y, width, height, offset) {
  ...
  height += 2 * offset;
  r += offset;
  let path = [['M', x + r, y], ['a', r, r, 0, 0, 0, -r, r], ['l', 0, height - 2 * r], ['a', r, r, 0, 0, 0, r, r], ['l', width - 2 * r, 0], ['a', r, r, 0, 0, 0, r, -r], ['l', 0, -height + 2 * r], ['a', r, r, 0, 0, 0, -r, -r], ['z']];
  return componentsToPath(path);
}
function getParticipantBandOutline(x, y, width, height, participantBandKind) {
  ...
  if (participantBandKind.startsWith('top')) {
    path = [['M', x, y + height], ['l', width, 0], ['l', 0, -height + r], ['a', r, r, 0, 0, 0, -r, -r], ['l', -width + 2 * r, 0], ['a', r, r, 0, 0, 0, -r, r], ['z']];
  } else if (participantBandKind.startsWith('bottom')) {
    path = [['M', x + width, y], ['l', -width, 0], ['l', 0, height - r], ['a', r, r, 0, 0, 0, r, r], ['l', width - 2 * r, 0], ['a', r, r, 0, 0, 0, r, -r], ['z']];
  } else {
    path = [['M', x, y + height], ['l', width, 0], ['l', 0, -height], ['l', -width, 0], ['z']];
  }
  return componentsToPath(path);
}
