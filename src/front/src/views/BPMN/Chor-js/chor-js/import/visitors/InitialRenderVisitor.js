...
import { isLabelExternal, getExternalLabelBounds } from 'bpmn-js/lib/util/LabelUtil';
import { getMid } from 'diagram-js/lib/layout/LayoutUtil';
import { resetAllBands } from '../../util/BandUtil';
import { createMessageShape, createMessageFlowSemantics, linkMessageFlowSemantics } from '../../util/MessageUtil';
let diRefs = new Refs({
  name: 'bpmnElement',
  enumerable: true
}, {
 ...
 */
export default function InitialRenderVisitor(injector, eventBus, canvas, elementFactory, elementRegistry, translate, textRenderer, moddle) {
  ...
  if (hasDI) {
    ...
    if (isChoreoActivity) {
      ...
      participants.forEach(participant => {
        participant.diBand = semantic.di.$parent.planeElement.find(diBand => diBand.choreographyActivityShape === di && diBand.bpmnElement === participant);
      });
    ...
    if (isChoreoTask) {
      ...
      if (needsAdditionalMessageFlow) {
        let messageFlow = createMessageFlowSemantics(this._injector, semantic, semantic.initiatingParticipantRef);
    ...
    if (is(di, 'bpmndi:BPMNPlane')) {
      ...
      let collapsed = is(semantic, 'bpmn:SubChoreography') || is(semantic, 'bpmn:CallChoreography') ? !semantic.di.isExpanded : false;
      ...
      element = this._elementFactory.createConnection(elementData(semantic, {
        ...
        source: source,
        target: target,
        waypoints: di.waypoint.map(waypoint => ({
          x: waypoint.x,
          y: waypoint.y
        }))
      }));
