...
import { getBPMNList, getBPMNInstanceList, getDmnList ,getERCList} from '@/api/externalResource'
...
export const useERCListData = (consortiumId: string) => {
    const {
        data: ERCList = [],
        isLoading,
        isError,
        ...
        ['ercList', consortiumId], // key，用于缓存和识别
        async () => await getERCList(consortiumId) // 异步请求
    );

    return [ERCList, { isLoading, isError, isSuccess }, refetch] as const;
}
