...
export const useAvailableMembers = (envId: string): [any[], () => void] => {
...
export const useFireflyData = (
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        if (!envId || !membershipId) return;

        const data = await getFireflyList(envId, null);
        if (ignore) return [[], () => {}];
        const filterData = data.filter(
          (item: any) => item.membership === membershipId,
        );
...
export const useAllFireflyData = (
...
): [any[], any[], any[], any[], any[], boolean, () => void] => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const tokenTasks = await getAllTokenElements(
        coreUrl,
        contractName,
        bpmnInstanceId,
      );
      ...
      if (tokenTasks) {
        setTokenTasks(
          tokenTasks.map((item: any) => {
            const tokenElementID =
              item.tokenElementID || item.TokenElementID || item.id;
            return {
              ...item,
              tokenElementID,
              TokenElementID: item.TokenElementID || tokenElementID,
...
export const useAvailableIdentity = () => {
  ...
  const { data, isLoading, isError, isSuccess, refetch } = useQuery(
    ["availableIdentity", currenOrgId, currenEnvId],
    async () => {
...
export const useFireflyIdentity = (coreUrl: string, idInFirefly: string) => {
  const { data, isLoading } = useQuery(
    [" fireflyIdentity", idInFirefly],
    async () => {
      const res = await axios.get(
        `${coreUrl}/api/v1/identities/${idInFirefly}/verifiers`,
      );
      return res.data;
    },
  );
