...
def clean_artifacts():
    ...
    print("[bpmn] cleaned node_modules/dist/.vite")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="bpmn_devtools.sh",
        description=(
            "BPMN choreographer dev helper for src/bpmn-chor-app.\n"
            "Use: ./bpmn_devtools.sh <command> [-- extra args]"
        ),
        formatter_class=argparse.RawTextHelpFormatter,
        epilog=(
            "Examples:\n"
            "  bpmn_devtools.sh setup\n"
            "  bpmn_devtools.sh dev\n"
            "  bpmn_devtools.sh build\n"
            "  bpmn_devtools.sh clean\n"
        ),
    )
