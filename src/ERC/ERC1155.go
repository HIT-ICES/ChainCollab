...
const burnerMSPsKey = "burnerMSPs"
...
func (s *SmartContract) Burn(ctx contractapi.TransactionContextInterface, account string, id uint64, amount uint64) error {
	...
	err = burnerAuthorizationHelper(ctx)
	if err != nil {
		return err
	}
...
func (s *SmartContract) Initialize(ctx contractapi.TransactionContextInterface, name string, symbol string,minterMSPs []string, burnerMSPs []string) (bool, error) {
	...
	err = ctx.GetStub().PutState(minterMSPsKey, minterMSPsJSON)
	if err != nil {
		return false, fmt.Errorf("failed to set minter MSPs: %v", err)
	}
	
	// 保存有权销毁代币的组织列表
	burnerMSPsJSON, err := json.Marshal(burnerMSPs)
	if err != nil {
		return false, fmt.Errorf("failed to marshal burner MSPs: %v", err)
	}
	err = ctx.GetStub().PutState(burnerMSPsKey, burnerMSPsJSON)
	if err != nil {
...
// 检查铸造权限 - 检查调用方是否在有权创建代币的组织列表中
func authorizationHelper(ctx contractapi.TransactionContextInterface) error {
	return checkMSPAuthorization(ctx, minterMSPsKey, "client is not authorized to mint new tokens")
}

// 检查销毁权限 - 检查调用方是否在有权销毁代币的组织列表中
func burnerAuthorizationHelper(ctx contractapi.TransactionContextInterface) error {
	return checkMSPAuthorization(ctx, burnerMSPsKey, "client is not authorized to burn tokens")
}

// 通用的MSP权限检查
func checkMSPAuthorization(ctx contractapi.TransactionContextInterface, mspListKey string, errorMsg string) error {
	clientMSPID, err := ctx.GetClientIdentity().GetMSPID()
	if err != nil {
	...
	// 获取授权组织列表
	mspsJSON, err := ctx.GetStub().GetState(mspListKey)
	if err != nil {
		return fmt.Errorf("failed to read authorized MSPs: %v", err)
	}
	
	if mspsJSON == nil {
		return fmt.Errorf("authorized MSPs not initialized")
	}
	
	var authorizedMSPs []string
	err = json.Unmarshal(mspsJSON, &authorizedMSPs)
	if err != nil {
		return fmt.Errorf("failed to unmarshal authorized MSPs: %v", err)
	}
	
	// 检查调用方是否在授权列表中
	for _, msp := range authorizedMSPs {
		if clientMSPID == msp {
			return nil
		}
	}
