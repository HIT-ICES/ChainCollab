...
contract GeneratedContract {
    ...
    function createInstance(
        ...
        _createMessage(
            ...
            ParticipantKey.Participant_0sa2v7d,
            ElementState.DISABLED,
            "requestId + numberOfUnits + urgent"
        );
        _createMessage(
            inst,
            ...
            ParticipantKey.Participant_0w6qkdf,
            ElementState.DISABLED,
            "{\"properties\":{},\"required\":[],\"files\":{\"deliveryNote\":{\"type\":\"file\",\"description\":\"A document confirming the delivery of goods\"}},\"file required\":[\"deliveryNote\"]}"
        );
        _createMessage(
            inst,
        ...
        _createMessage(
            ...
            ParticipantKey.Participant_19j1e3o,
            ElementState.DISABLED,
            "{\"properties\":{},\"required\":[],\"files\":{\"waybill\":{\"type\":\"file\",\"description\":\"Digital or physical waybill document\uff0cdetailing the shipment contents and destination.\"}},\"file required\":[\"waybill\"]}"
        );
        _createMessage(
            inst,
...
function Message_1dmeexg_Send(uint256 instanceId, string calldata fireflyTranId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.Message_1dmeexg];
        require(m.exists, "message not set");
        _checkParticipant(inst, m.sendParticipant);
...
function Message_1dzkcn0_Send(uint256 instanceId, string calldata fireflyTranId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.Message_1dzkcn0];
        require(m.exists, "message not set");
        _checkParticipant(inst, m.sendParticipant);
...
function Message_1oxmq1k_Send(uint256 instanceId, string calldata fireflyTranId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.Message_1oxmq1k];
        require(m.exists, "message not set");
        _checkParticipant(inst, m.sendParticipant);
