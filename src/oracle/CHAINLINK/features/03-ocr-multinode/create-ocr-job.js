...
async function createOCRJobForNode(node) {
  try {
    ...
    let jobSpec;
    if (node.isBootstrap) {
      jobSpec = fs.readFileSync(path.join(__dirname, 'job-spec-ocr-bootstrap.toml'), 'utf8');
    } else {
      jobSpec = fs.readFileSync(path.join(__dirname, 'job-spec-ocr.toml'), 'utf8');
    }
    ...
    if (fs.existsSync(path.join(deploymentDir, 'ocr-deployment.json'))) {
      const ocrDeployment = JSON.parse(fs.readFileSync(path.join(deploymentDir, 'ocr-deployment.json'), 'utf8'));
      // 确保合约地址是 EIP55 格式
      try {
...
async function main() {
  ...
  let deploymentData = {};
  const deploymentFile = path.join(deploymentDir, 'chainlink-deployment.json');
