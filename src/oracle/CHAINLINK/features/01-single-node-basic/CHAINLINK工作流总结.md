...
### 2.1 Job Spec 配置 (features/01-single-node-basic/job-spec.toml)
...
3. `<mybootnode-container>`: Geth 私链节点
...
4. 动态替换 `features/01-single-node-basic/job-spec.toml` 中的硬编码地址
...
if [ -z "$BALANCE" ] || [ "$BALANCE" = "0" ]; then
    ...
    # 启动挖矿（如果需要）
    docker exec <mybootnode-container> geth attach /root/.ethereum/geth.ipc --exec "miner.start()"
    # 转账 1000 ETH
    docker exec <mybootnode-container> geth attach /root/.ethereum/geth.ipc --exec "eth.sendTransaction({from: eth.coinbase, to: '$GETH_ACCOUNT', value: web3.toWei(1000, 'ether')})"
fi
```
...
----------------------------------------------------------------------------------------
<mybootnode-container>   geth --http --http.addr=0.0 ...   Up (healthy)   0.0.0.0:8545->8545/tcp, 0.0.0.0:30303->30303/tcp
...
**特点**:
- 使用硬编码凭证登录
- 读取 features/01-single-node-basic/job-spec.toml 配置
