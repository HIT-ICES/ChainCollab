...
contract OffchainAggregator is Owned, OffchainAggregatorBilling, AggregatorV2V3Interface, TypeAndVersionInterface {
    ...
    external
  {
    ...
    { // Check the report contents, and record the result
      ...
      require(
        r.observations[0] == r.observations[r.observations.length - 1],
        "observations not equal"
      );
      int192 answer = r.observations[0];
      require(minAnswer <= answer && answer <= maxAnswer, "answer is out of min-max range");
      r.hotVars.latestAggregatorRoundId++;
      s_transmissions[r.hotVars.latestAggregatorRoundId] =
        Transmission(answer, uint64(block.timestamp));
      ...
      validateAnswer(r.hotVars.latestAggregatorRoundId, answer);
    }
    s_hotVars = r.hotVars;
