...
interface IMainDataTaskCallback {
...
/// @notice 数据任务子合约：由主合约驱动发起 Chainlink 请求，并将结果回调给主合约。
contract ChainlinkDataTaskAdapter is ChainlinkClient, ConfirmedOwner {
    ...
    event DataTaskRequested(
        bytes32 indexed taskId,
        bytes32 indexed requestId,
        string sourceUrl,
        string query
    );
    event DataTaskCommitted(
        bytes32 indexed taskId,
        bytes32 indexed requestId,
        string raw
    );
    ...
    function requestDataTask(
        ...
        req._add("sourceUrl", sourceUrl);
        req._add("query", query);
        ...
        emit DataTaskRequested(taskId, requestId, sourceUrl, query);
    }
    ...
    function _commit(bytes32 requestId, string memory raw) internal {
        ...
        IMainDataTaskCallback(controller).onDataTaskResult(
            pending.taskId,
            requestId,
            raw
        );
        emit DataTaskCommitted(pending.taskId, requestId, raw);
    }
}
