...
function extractObservationURL() {
  ...
  const path = require('path');
  const specPath = path.resolve(__dirname, 'job-spec-ocr.toml');
...
async function main() {
  const nodeInfo = readJson(path.join(ROOT_DIR, 'deployment', 'node-info.json'), 'node-info.json');
  const ocrDeployment = readJson(path.join(ROOT_DIR, 'deployment', 'ocr-deployment.json'), 'ocr-deployment.json');
