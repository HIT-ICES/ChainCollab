...
import shlex
import subprocess
from common.lib.peer.command import Command
from apps.api.config import FABRIC_TOOL, FABRIC_CFG
...
class ChainCode(Command):
    ...
    def lifecycle_approve_for_my_org(
        self,
        orderer_url,
        ...
        try:
            ...
            if package_id == "":
                return (
                    1,
                    "not exist the chaincode, please check chaincode_name and chaincode_version",
                )
    ...
    def lifecycle_check_commit_readiness(
        self,
        orderer_url,
        orderer_tls_rootcert,
        channel_name,
    ...
    def lifecycle_commit(
        ...
        try:
            ...
            command_str_with_tls = (
                "{} lifecycle chaincode commit -o {} --tls --cafile {} "
                "--channelID {} --name {} --version {} --sequence {} "
            )
            command_str_without_tls = (
                "{} lifecycle chaincode commit -o {} --channelID {} --name {} "
                "--version {} --sequence {} "
            )
    ...
    def invoke(
        self, orderer_url, orderer_tls_rootcert, channel_name, cc_name, args, init=False
    ):
        """
        :param orderer_url:orderer accessable url
        :param orderer_tls_rootcert: orderer tls certificate
        ...
        try:
            if init:
                invoke_command = (
                    "{} chaincode invoke -I -o {} --channelID {} --name {} -c '{}'"
                )
                invoke_command_tls = "{} chaincode invoke -I -o {} --tls --cafile {} --channelID {} --name {} -c '{}'"
            else:
                invoke_command = (
                    "{} chaincode invoke -o {} --channelID {} --name {} -c '{}'"
                )
