...
from apps.fabric.models import Channel
...
class ChannelUpdateSerializer(serializers.Serializer):
    msp_id = serializers.CharField(max_length=128, help_text="MSP ID of Organization")
...
class ChannelOrgListSerializer(serializers.Serializer):
    ...
    name = serializers.CharField(max_length=128, help_text="name of Organization")
...
class ChannelAnchorBody(serializers.Serializer):
    anchor_peers = serializers.ListField(
        child=serializers.UUIDField(help_text="ID of Anchor Peer Nodes")
    )
    orderers = serializers.ListField(
        child=serializers.UUIDField(help_text="ID of Orderer Nodes")
    )
    resource_set_id = serializers.UUIDField(help_text="ID of Resource Set")
