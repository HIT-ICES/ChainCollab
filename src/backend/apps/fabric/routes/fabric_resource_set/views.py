...
from common.exceptions import ResourceExists, ResourceNotFound, ResourceInUse
from apps.fabric.models import Node, FabricResourceSet
from apps.fabric.routes.fabric_resource_set.serializers import (
    OrganizationQuery,
...
from apps.core.routes.user.serializers import UserIDSerializer
from apps.core.models import UserProfile
from apps.core.routes.user.serializers import UserListSerializer, UserQuerySerializer
...
from common.lib.ca.ca import FabricCA
...
class FabricResourceSetViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            organizations = FabricResourceSet.objects.filter(**parameters)
            ...
            organizations = [
                {
                    ...
                    "network": str(organization.network.id)
            ...
            response = OrganizationList(data={"total": p.count, "data": organizations})
    ...
    def create(self, request):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            try:
                FabricResourceSet.objects.get(name=name)
            ...
            organization = FabricResourceSet(name=name, msp=msp, tls=tls)
            organization.save()
    ...
    def destroy(self, request, pk=None):
        ...
        """
        try:
            organization = FabricResourceSet.objects.get(id=pk)
            if organization.network:
                raise ResourceInUse
    ...
    @swagger_auto_schema(
        responses=with_common_response({status.HTTP_200_OK: OrganizationResponse})
    )
    ...
    def _list_users(request, pk=None):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            response = UserListSerializer(data={"total": p.count, "data": users})
            if response.is_valid(raise_exception=True):
                return Response(ok(response.validated_data), status=status.HTTP_200_OK)
    ...
    @swagger_auto_schema(
        ...
        responses=with_common_response({status.HTTP_200_OK: UserListSerializer}),
    )
    @swagger_auto_schema(
        method="post",
    ...
    @action(methods=["delete"], detail=True, url_path="users/(?P<user_id>[^/.]+)")
