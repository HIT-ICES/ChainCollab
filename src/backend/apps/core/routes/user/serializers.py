...
from common.enums import Operation, NetworkType, FabricNodeType, UserRole
from common.serializers import PageQuerySerializer
from apps.core.models import UserProfile
from common.utils.jwt import FabricResourceSetSerializer
...
class UserQuerySerializer(PageQuerySerializer, serializers.Serializer):
    ...
    org_id = serializers.UUIDField(
        help_text="Organization ID to filter", required=False
    )


class UserInfoSerializer(serializers.ModelSerializer):
    id = serializers.UUIDField(help_text="ID of user")
    # organization = FabricResourceSetSerializer(allow_null=True, required=False)
...
class UserAuthSerializer(serializers.Serializer):
    username = serializers.CharField(help_text="Username for login", max_length=64)
    password = serializers.CharField(help_text="Password for login", max_length=64)
...
class UserUpdateSerializer(serializers.Serializer):
    password = serializers.CharField(help_text="New password for login", max_length=64)
