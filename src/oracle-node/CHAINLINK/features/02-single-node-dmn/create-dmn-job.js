...
// Job 配置文件路径
const JOB_SPEC_PATH = path.resolve(__dirname, 'job-spec-dmn-java.toml');
...
async function main() {
    try {
        ...
        if (!fs.existsSync(path.join(deploymentDir, 'chainlink-deployment.json'))) {
            console.error('❌ 找不到 chainlink-deployment.json 文件，请先部署 Chainlink 基础设施');
            process.exit(1);
        }

        // 读取 chainlink-deployment.json 文件
        const chainlinkDeployment = JSON.parse(fs.readFileSync(path.join(deploymentDir, 'chainlink-deployment.json'), 'utf8'));
        ...
        console.log('   cd oracle-node/CHAINLINK/features/02-single-node-dmn');
        console.log('   ./start-dmn-server-java.sh');
        console.log('');
        ...
        console.log('   node features/02-single-node-dmn/test-dmn-oracle.js');
        console.log('');

        // 更新部署信息
        const deploymentPath = path.join(deploymentDir, 'chainlink-deployment.json');
