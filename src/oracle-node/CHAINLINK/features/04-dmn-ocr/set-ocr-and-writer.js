...
function findBootstrapWriter() {
  if (!Array.isArray(nodeInfo)) return null;
  const bootstrap = nodeInfo.find((node) => node.name === 'bootstrap');
  return bootstrap?.ethAddress || null;
}
...
async function main() {
  ...
  const writer = process.env.RAW_WRITER || findBootstrapWriter();
  ...
  if (!writer) {
    console.error('缺少 writer 地址：deployment/node-info.json 或 RAW_WRITER');
    process.exit(1);
  }

  console.log('合约地址:', contractAddress);
  console.log('OCR aggregator:', ocrAggregator);
  console.log('raw writer:', writer);

  const tx1 = await sendTx(contractAddress, 'setOcrAggregator', [ocrAggregator], 200000);
  console.log('✅ setOcrAggregator 交易已发送:', tx1);

  const tx2 = await sendTx(contractAddress, 'setRawWriter', [writer], 200000);
  console.log('✅ setRawWriter 交易已发送:', tx2);
}
