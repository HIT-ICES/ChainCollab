...
docker-compose -f docker-compose-multinode.yml up -d postgres-bootstrap mybootnode chainlink-bootstrap
...
for i in {1..30}; do
    ...
    node scripts/get-node-info.js bootstrap > /dev/null 2>&1
    if [ -f deployment/node-info.json ]; then
        BOOTSTRAP_PEER_ID=$(jq -r '.[] | select(.name == "bootstrap") | .p2pPeerId' deployment/node-info.json)
...
for node in chainlink1 chainlink2 chainlink3 chainlink4; do
    ...
    python3 - "$node/config.toml" "$BOOTSTRAP_MULTIADDR" <<'PY'
import re
import sys
...
docker-compose -f docker-compose-multinode.yml up -d chainlink1 chainlink2 chainlink3 chainlink4
...
for node in chainlink1 chainlink2 chainlink3 chainlink4; do
    ...
    sed -i "s|AnnounceAddresses = \\[\".*\"\\]|AnnounceAddresses = [\"${BOOTSTRAP_IP}:${PORT}\"]|g" $node/config.toml
done
docker-compose -f docker-compose-multinode.yml restart chainlink1 chainlink2 chainlink3 chainlink4
