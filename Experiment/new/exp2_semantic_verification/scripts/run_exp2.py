...
def run_solc_ast(solidity_path: Path, output_path: Path) -> None:
    ...
    input_path = output_path.with_suffix(".solc.input.json")
...
def process_case(case_info: Dict[str, Any], results_dir: Path, *, backend: str, api_base_url: str | None) -> Dict[str, Any]:
    ...
    b2c_path = case_dir / f"{case_info['case_name']}.b2c"
    go_path = case_dir / f"{case_info['case_name']}.go"
    sol_path = case_dir / f"{case_info['case_name']}.sol"
    dsl_ast_path = case_dir / "dsl_ast.json"
    go_ast_path = case_dir / "go_ast.json"
    sol_ast_path = case_dir / "sol_ast.json"
...
def main() -> None:
    ...
    for case_root in discover_case_dirs(cases_dir):
        ...
        try:
            run_log.append(process_case(case_info, results_dir, backend=args.backend, api_base_url=args.api_base_url))
    ...
    run_python(EXP2_ROOT / "scripts" / "summary_report.py", ["--results-dir", str(results_dir)])
