...
from mapping_rules import (
    ...
    verify_global_sol,
    verify_message_sol,
    verify_oracletask_sol,
    verify_participant_sol,
)
...
def build_report(case_name: str, dsl_ast: Dict[str, Any], sol_ast: Dict[str, Any]) -> Dict[str, Any]:
    results: List[Dict[str, Any]] = []
    for item in dsl_ast.get("globals", []):
        append_result(results, item["name"], "global", verify_global_sol(item, sol_ast))
    for item in dsl_ast.get("participants", []):
        append_result(results, item["name"], "participant", verify_participant_sol(item, sol_ast))
    for item in dsl_ast.get("messages", []):
        append_result(results, item["name"], "message", verify_message_sol(item, sol_ast))
    for item in dsl_ast.get("gateways", []):
        append_result(results, item["name"], "gateway", verify_gateway_sol(item, sol_ast))
    for item in dsl_ast.get("events", []):
        append_result(results, item["name"], "event", verify_event_sol(item, sol_ast))
    for item in dsl_ast.get("businessrules", []):
        append_result(results, item["name"], "businessrule", verify_businessrule_sol(item, sol_ast))
    for item in dsl_ast.get("oracletasks", []):
        append_result(results, item["name"], "oracletask", verify_oracletask_sol(item, sol_ast))
    for index, flow in enumerate(dsl_ast.get("flows", []), start=1):
        trigger = flow.get("trigger", {})
        label = f"flow_{index}:{trigger.get('type')}:{trigger.get('name')}"
        append_result(results, label, "flow", verify_flow_sol(flow, sol_ast))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Verify DSL -> Solidity semantic preservation.")
    parser.add_argument("--dsl-ast", required=True, help="Path to dsl_ast.json")
    parser.add_argument("--sol-ast", required=True, help="Path to sol_ast.json")
    ...
    sol_ast = load_json(Path(args.sol_ast).resolve())
    report = build_report(args.case_name or Path(args.output).resolve().parent.name, dsl_ast, sol_ast)
