...
contract Coffee_machine {
    ...
    struct InitParameters {
        ...
        BusinessRuleInit Activity_0ysk2q6;
    }
    ...
    function createInstance(
        ...
        _createBusinessRule(
            inst,
            BusinessRuleKey.Activity_0ysk2q6,
            params.Activity_0ysk2q6
        );
        ...
        )
    {
        ...
        messageStates = new uint8[](5);
        messageFireflyTranIds = new string[](5);
        {
        ...
        {
            BusinessRule storage br = inst.businessRules[BusinessRuleKey.Activity_0ysk2q6];
            businessRuleStates[0] = uint8(br.state);
            businessRuleRequestIds[0] = br.requestId;
        }
    }

    function Activity_0ysk2q6(uint256 instanceId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        BusinessRule storage br = inst.businessRules[BusinessRuleKey.Activity_0ysk2q6];
        ...
        emit BusinessRuleRequested(instanceId, BusinessRuleKey.Activity_0ysk2q6, requestId);
    }

    function Activity_0ysk2q6_Continue(uint256 instanceId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        BusinessRule storage br = inst.businessRules[BusinessRuleKey.Activity_0ysk2q6];
        require(br.exists, "business rule not set");
        ...
        emit BusinessRuleCompleted(instanceId, BusinessRuleKey.Activity_0ysk2q6, br.requestId);
    }
...
function Message_0gg08bf_Send(uint256 instanceId, string calldata fireflyTranId) external onlyInitialized {
        ...
        inst.messages[MessageKey.Message_1j4s0qh].state = ElementState.ENABLED;
...
function Message_0i0xp6a_Send(uint256 instanceId, string calldata fireflyTranId) external onlyInitialized {
        ...
        inst.messages[MessageKey.Message_0gg08bf].state = ElementState.ENABLED;

    }

function Message_1e90tfn_Send(uint256 instanceId, string calldata fireflyTranId, bool error) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.Message_1e90tfn];
        require(m.exists, "message not set");
        _checkParticipant(inst, m.sendParticipant);
        ...
        m.fireflyTranId = fireflyTranId;
        inst.stateMemory.Error = error;
        m.state = ElementState.COMPLETED;
        emit MessageSent(instanceId, MessageKey.Message_1e90tfn, fireflyTranId);
        inst.messages[MessageKey.Message_1uiozoi].state = ElementState.ENABLED;
...
function Message_1j4s0qh_Send(uint256 instanceId, string calldata fireflyTranId) external onlyInitialized {
        ...
        inst.gateways[GatewayKey.ExclusiveGateway_0c8hy9b].state = ElementState.ENABLED;
...
function Message_1uiozoi_Send(uint256 instanceId, string calldata fireflyTranId) external onlyInitialized {
        ...
        inst.messages[MessageKey.Message_0i0xp6a].state = ElementState.ENABLED;
...
function ExclusiveGateway_1sp1v7s(uint256 instanceId) external onlyInitialized {
        ...
        inst.businessRules[BusinessRuleKey.Activity_0ysk2q6].state = ElementState.ENABLED;
