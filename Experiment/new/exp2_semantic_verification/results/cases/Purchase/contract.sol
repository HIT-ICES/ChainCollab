...
contract GeneratedContract {
    ...
    struct InitParameters {
        ...
        BusinessRuleInit Activity_12arovy;
    }
    ...
    function createInstance(
        ...
        _createGateway(
            inst,
            GatewayKey.Gateway_13i0b7w,
            ElementState.DISABLED
        );

        _createBusinessRule(
            inst,
            BusinessRuleKey.Activity_12arovy,
            params.Activity_12arovy
        );
        ...
        )
    {
        ...
        {
            Message storage m = inst.messages[MessageKey.Message_08ms1jj];
        ...
        {
            Message storage m = inst.messages[MessageKey.Message_15b83n1];
        ...
        {
            Gateway storage g = inst.gateways[GatewayKey.Gateway_13i0b7w];
        ...
        {
            BusinessRule storage br = inst.businessRules[BusinessRuleKey.Activity_12arovy];
            businessRuleStates[0] = uint8(br.state);
            businessRuleRequestIds[0] = br.requestId;
        }
    }

    function Activity_12arovy(uint256 instanceId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        BusinessRule storage br = inst.businessRules[BusinessRuleKey.Activity_12arovy];
        ...
        inputBody = _jsonAppendField(
            ...
            _jsonField(
                "Price",
            _jsonInt(inst.stateMemory.Price)
            )
        );
        ...
        emit BusinessRuleRequested(instanceId, BusinessRuleKey.Activity_12arovy, requestId);
    }

    function Activity_12arovy_Continue(uint256 instanceId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        BusinessRule storage br = inst.businessRules[BusinessRuleKey.Activity_12arovy];
        require(br.exists, "business rule not set");
        ...
        inst.stateMemory.Discount = _extractJsonBool(raw, "discount");
        br.state = ElementState.COMPLETED;
        inst.gateways[GatewayKey.Gateway_13i0b7w].state = ElementState.ENABLED;


        emit BusinessRuleCompleted(instanceId, BusinessRuleKey.Activity_12arovy, br.requestId);
    }
...
function Message_05gz8u0_Send(uint256 instanceId, string calldata fireflyTranId, string calldata MessageContent) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.Message_05gz8u0];
        ...
        inst.stateMemory.MessageContent = MessageContent;
        m.state = ElementState.COMPLETED;
        emit MessageSent(instanceId, MessageKey.Message_05gz8u0, fireflyTranId);
        inst.events[EventKey.Event_0e3j88g].state = ElementState.ENABLED;

    }

function Message_08ms1jj_Send(uint256 instanceId, string calldata fireflyTranId, string calldata sale) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.Message_08ms1jj];
        require(m.exists, "message not set");
        _checkParticipant(inst, m.sendParticipant);
        ...
        m.fireflyTranId = fireflyTranId;
        inst.stateMemory.Sale = sale;
        m.state = ElementState.COMPLETED;
        emit MessageSent(instanceId, MessageKey.Message_08ms1jj, fireflyTranId);
        inst.messages[MessageKey.Message_0g7m9tf].state = ElementState.ENABLED;

    }

function Message_0g7m9tf_Send(uint256 instanceId, string calldata fireflyTranId, string calldata num, string calldata time2) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.Message_0g7m9tf];
        require(m.exists, "message not set");
        _checkParticipant(inst, m.sendParticipant);
        ...
        m.fireflyTranId = fireflyTranId;
        inst.stateMemory.Num = num;
        inst.stateMemory.Time2 = time2;
        m.state = ElementState.COMPLETED;
        emit MessageSent(instanceId, MessageKey.Message_0g7m9tf, fireflyTranId);
        inst.events[EventKey.Event_1o9guxu].state = ElementState.ENABLED;

    }

function Message_0mcktm6_Send(uint256 instanceId, string calldata fireflyTranId, int256 Price, string calldata Time) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.Message_0mcktm6];
        require(m.exists, "message not set");
        _checkParticipant(inst, m.sendParticipant);
        ...
        m.fireflyTranId = fireflyTranId;
        inst.stateMemory.Price = Price;
        inst.stateMemory.Time = Time;
        m.state = ElementState.COMPLETED;
        emit MessageSent(instanceId, MessageKey.Message_0mcktm6, fireflyTranId);
        inst.businessRules[BusinessRuleKey.Activity_12arovy].state = ElementState.ENABLED;

    }

function Message_0q9hvem_Send(uint256 instanceId, string calldata fireflyTranId, string calldata orderContent, bool productAvalibale) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.Message_0q9hvem];
        require(m.exists, "message not set");
        _checkParticipant(inst, m.sendParticipant);
        ...
        inst.stateMemory.ProductAvalibale = productAvalibale;
        m.state = ElementState.COMPLETED;
        emit MessageSent(instanceId, MessageKey.Message_0q9hvem, fireflyTranId);
        inst.gateways[GatewayKey.Gateway_1ltys0e].state = ElementState.ENABLED;

    }

function Message_14wu8ts_Send(uint256 instanceId, string calldata fireflyTranId, string calldata MessageContent) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.Message_14wu8ts];
        require(m.exists, "message not set");
        _checkParticipant(inst, m.sendParticipant);
        ...
        inst.events[EventKey.Event_194zr5n].state = ElementState.ENABLED;

    }

function Message_15b83n1_Send(uint256 instanceId, string calldata fireflyTranId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.Message_15b83n1];
        require(m.exists, "message not set");
        _checkParticipant(inst, m.sendParticipant);
        ...
        m.fireflyTranId = fireflyTranId;
        m.state = ElementState.COMPLETED;
        emit MessageSent(instanceId, MessageKey.Message_15b83n1, fireflyTranId);
        inst.messages[MessageKey.Message_08ms1jj].state = ElementState.ENABLED;

    }

function Message_1f0gefc_Send(uint256 instanceId, string calldata fireflyTranId, string calldata MessageContent, bool priceOK) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.Message_1f0gefc];
        require(m.exists, "message not set");
        _checkParticipant(inst, m.sendParticipant);
        ...
        m.fireflyTranId = fireflyTranId;
        inst.stateMemory.MessageContent = MessageContent;
        inst.stateMemory.PriceOK = priceOK;
        m.state = ElementState.COMPLETED;
        emit MessageSent(instanceId, MessageKey.Message_1f0gefc, fireflyTranId);
        inst.gateways[GatewayKey.Gateway_025uwvp].state = ElementState.ENABLED;
...
function Gateway_1ltys0e(uint256 instanceId) external onlyInitialized {
        ...
        if (inst.stateMemory.ProductAvalibale == true) {
            inst.messages[MessageKey.Message_1f0gefc].state = ElementState.ENABLED;
        }
        else if (inst.stateMemory.ProductAvalibale == false) {
...
function Gateway_025uwvp(uint256 instanceId) external onlyInitialized {
        ...
        if (inst.stateMemory.PriceOK == true) {
            inst.messages[MessageKey.Message_0mcktm6].state = ElementState.ENABLED;
        }
        else if (inst.stateMemory.PriceOK == false) {
            inst.messages[MessageKey.Message_05gz8u0].state = ElementState.ENABLED;
        }

    }

function Gateway_13i0b7w(uint256 instanceId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Gateway storage g = inst.gateways[GatewayKey.Gateway_13i0b7w];
        require(g.exists, "gateway not set");
        require(g.state == ElementState.ENABLED, "gateway state not allowed");
        ...
        g.state = ElementState.COMPLETED;
        emit GatewayDone(instanceId, GatewayKey.Gateway_13i0b7w);
        if (inst.stateMemory.Discount == false) {
            inst.messages[MessageKey.Message_0g7m9tf].state = ElementState.ENABLED;
        }
        else if (inst.stateMemory.Discount == true) {
            inst.messages[MessageKey.Message_15b83n1].state = ElementState.ENABLED;
        }
