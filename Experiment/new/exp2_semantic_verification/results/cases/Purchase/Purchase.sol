...
contract Purchase {
        ...
        )
    {
        ...
        messageStates = new uint8[](7);
        messageFireflyTranIds = new string[](7);
        {
...
function Message_0g7m9tf_Send(uint256 instanceId, string calldata fireflyTranId, string calldata num, string calldata time) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.Message_0g7m9tf];
        require(m.exists, "message not set");
        _checkParticipant(inst, m.sendParticipant);
