...
contract customer {
    ...
    enum EventKey {
        Event_1jtgn3j,        Event_0366pfz,        Event_08edp7f,        Event_146eii4,        Event_143ykco    }
    ...
    struct StateMemory {
        ...
        string InvoiceType;
        bool InvoiceTypeAvailable;
        int256 Invoice_data;
        string Invoice_id;
        bool Is_available;
        int256 MembershipId;
        int256 MembershipLevel;
        string Motivation;
    ...
    function Activity_1yl9tfp(uint256 instanceId) external onlyInitialized {
        ...
        inputBody = _jsonAppendField(
            ...
            _jsonField(
                "membershipId",
            _jsonInt(inst.stateMemory.MembershipId)
            )
        );
    ...
    function Activity_1yl9tfp_Continue(uint256 instanceId) external onlyInitialized {
        ...
        inst.stateMemory.MembershipLevel = _extractJsonInt(raw, "membershipLevel");
    ...
    function Activity_0ibsbry_Continue(uint256 instanceId) external onlyInitialized {
        ...
        br.lastRawResult = raw;
        br.fulfilledAt = fulfilledAt;
        inst.stateMemory.InvoiceTypeAvailable = _extractJsonBool(raw, "invoiceTypeAvailable");
        br.state = ElementState.COMPLETED;
...
function Message_045i10y_Send(uint256 instanceId, string calldata fireflyTranId, string calldata serviceId, int256 membershipId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.Message_045i10y];
        ...
        inst.stateMemory.MembershipId = membershipId;
...
function ExclusiveGateway_0nzwv7v(uint256 instanceId) external onlyInitialized {
        ...
        else if (inst.stateMemory.Invoice == false) {
...
function Gateway_1bhtapl(uint256 instanceId) external onlyInitialized {
        ...
        if (inst.stateMemory.Need_external_provider == false) {
