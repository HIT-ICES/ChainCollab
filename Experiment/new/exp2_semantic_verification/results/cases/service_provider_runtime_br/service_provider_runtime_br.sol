...
contract service_provider_runtime_br {
    enum ElementState {
        DISABLED,
        ENABLED,
    ...
    enum EventKey {
        Event_1jtgn3j,        Event_0366pfz,        Event_08edp7f,        Event_146eii4    }
    ...
    struct InitParameters {
        ...
        BusinessRuleInit Activity_1q19lty;
    }
    ...
    function createInstance(
        ...
        _createBusinessRule(
            inst,
            BusinessRuleKey.Activity_1q19lty,
            params.Activity_1q19lty
        );
        ...
        )
    {
        ...
        {
            ActionEvent storage ev = inst.events[EventKey.Event_08edp7f];
        ...
        {
            BusinessRule storage br = inst.businessRules[BusinessRuleKey.Activity_1q19lty];
            businessRuleStates[0] = uint8(br.state);
            businessRuleRequestIds[0] = br.requestId;
        }
    }

    function Activity_1q19lty(uint256 instanceId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        BusinessRule storage br = inst.businessRules[BusinessRuleKey.Activity_1q19lty];
        ...
        inputBody = _jsonAppendField(
            ...
            _jsonField(
                "temperature",
            _jsonInt(inst.stateMemory.Temperature)
            )
        );
        inputBody = _jsonAppendField(
            inputBody,
            _jsonField(
                "dataType",
            _jsonString(inst.stateMemory.DataType)
            )
        );
        ...
        emit BusinessRuleRequested(instanceId, BusinessRuleKey.Activity_1q19lty, requestId);
    }

    function Activity_1q19lty_Continue(uint256 instanceId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        BusinessRule storage br = inst.businessRules[BusinessRuleKey.Activity_1q19lty];
        require(br.exists, "business rule not set");
        ...
        inst.stateMemory.TestOutput = _extractJsonString(raw, "testOutput");
        br.state = ElementState.COMPLETED;
        inst.gateways[GatewayKey.Gateway_1bhtapl].state = ElementState.ENABLED;


        emit BusinessRuleCompleted(instanceId, BusinessRuleKey.Activity_1q19lty, br.requestId);
    }
...
function Message_045i10y_Send(uint256 instanceId, string calldata fireflyTranId, string calldata serviceId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.Message_045i10y];
...
function Message_0m9p3da_Send(uint256 instanceId, string calldata fireflyTranId, bool invoice) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.Message_0m9p3da];
        require(m.exists, "message not set");
        _checkParticipant(inst, m.sendParticipant);
...
function Message_0o8eyir_Send(uint256 instanceId, string calldata fireflyTranId, int256 payment_amount, int256 orderID, int256 temperature, string calldata dataType) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.Message_0o8eyir];
        require(m.exists, "message not set");
        _checkParticipant(inst, m.sendParticipant);
        ...
        inst.stateMemory.Temperature = temperature;
        inst.stateMemory.DataType = dataType;
        m.state = ElementState.COMPLETED;
        ...
        inst.businessRules[BusinessRuleKey.Activity_1q19lty].state = ElementState.ENABLED;
...
function ExclusiveGateway_0nzwv7v(uint256 instanceId) external onlyInitialized {
        ...
        if (inst.stateMemory.Invoice == false) {
            inst.events[EventKey.Event_08edp7f].state = ElementState.ENABLED;
        }
        else if (inst.stateMemory.Invoice == true) {
...
function Gateway_1bhtapl(uint256 instanceId) external onlyInitialized {
        ...
        if (inst.stateMemory.Need_external_provider == true) {
            inst.messages[MessageKey.Message_1i8rlqn].state = ElementState.ENABLED;
        }
        else if (inst.stateMemory.Need_external_provider == false) {
            inst.gateways[GatewayKey.Gateway_04h9e6e].state = ElementState.ENABLED;
        }
...
function Event_08edp7f(uint256 instanceId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        ActionEvent storage ev = inst.events[EventKey.Event_08edp7f];
        require(ev.exists, "event not set");
        require(ev.state == ElementState.ENABLED, "event state not allowed");
        ...
        ev.state = ElementState.COMPLETED;
        emit ActionEventDone(instanceId, EventKey.Event_08edp7f);
