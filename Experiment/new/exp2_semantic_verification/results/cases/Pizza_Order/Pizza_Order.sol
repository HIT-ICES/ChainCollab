...
contract Pizza_Order {
    ...
    struct InitParameters {
        ...
        string dmnEvalUrl;
        bool enforceBusinessRuleCaller;
        address Participant_1ssz56y_account;
        string Participant_1ssz56y_org;
        address Participant_1f296fx_account;
        string Participant_1f296fx_org;
        address Participant_0szm9kv_account;
        string Participant_0szm9kv_org;
        BusinessRuleInit Activity_0pzravo;
    }
    ...
    function createInstance(
        ...
        _createParticipant(
            inst,
            ParticipantKey.Participant_1ssz56y,
            params.Participant_1ssz56y_account,
            params.Participant_1ssz56y_org,
            false,
            0,
            0
        );
        _createParticipant(
            inst,
            ParticipantKey.Participant_1f296fx,
            params.Participant_1f296fx_account,
            params.Participant_1f296fx_org,
            false,
            0,
            0
        );
        _createParticipant(
            inst,
            ParticipantKey.Participant_0szm9kv,
            params.Participant_0szm9kv_account,
            params.Participant_0szm9kv_org,
            false,
            0,
            0
        );
        ...
        _createActionEvent(
            inst,
            EventKey.Event_17c6iyi,
            ElementState.ENABLED
        );
        _createActionEvent(
            inst,
            EventKey.Event_1bxai2s,
            ElementState.DISABLED
        );

        _createMessage(
            inst,
            MessageKey.Message_0gm7yco,
            ParticipantKey.Participant_1f296fx,
            ParticipantKey.Participant_1ssz56y,
            ElementState.DISABLED
        );
        ...
        _createGateway(
            inst,
            GatewayKey.Gateway_0ckb278,
            ElementState.DISABLED
        );
        _createGateway(
            inst,
            GatewayKey.Gateway_18xv5wc,
            ElementState.DISABLED
        );
        _createGateway(
            inst,
            GatewayKey.Gateway_1rwnujh,
            ElementState.DISABLED
        );
        _createGateway(
            inst,
            GatewayKey.Gateway_1k9qf30,
            ElementState.DISABLED
        );

        _createBusinessRule(
            inst,
            BusinessRuleKey.Activity_0pzravo,
            params.Activity_0pzravo
        );
        ...
        )
    {
        ...
        {
            Gateway storage g = inst.gateways[GatewayKey.Gateway_0ckb278];
            gatewayStates[0] = uint8(g.state);
        }
        {
            Gateway storage g = inst.gateways[GatewayKey.Gateway_18xv5wc];
            gatewayStates[1] = uint8(g.state);
        }
        {
            Gateway storage g = inst.gateways[GatewayKey.Gateway_1rwnujh];
            gatewayStates[2] = uint8(g.state);
        }
        {
            Gateway storage g = inst.gateways[GatewayKey.Gateway_1k9qf30];
            gatewayStates[3] = uint8(g.state);
        }

        eventStates = new uint8[](2);
        {
            ActionEvent storage ev = inst.events[EventKey.Event_17c6iyi];
            eventStates[0] = uint8(ev.state);
        }
        {
            ActionEvent storage ev = inst.events[EventKey.Event_1bxai2s];
        ...
        {
            BusinessRule storage br = inst.businessRules[BusinessRuleKey.Activity_0pzravo];
            businessRuleStates[0] = uint8(br.state);
            businessRuleRequestIds[0] = br.requestId;
        }
    }

    function Activity_0pzravo(uint256 instanceId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        BusinessRule storage br = inst.businessRules[BusinessRuleKey.Activity_0pzravo];
        ...
        inputBody = _jsonAppendField(
            ...
            _jsonField(
                "Address",
            _jsonString(inst.stateMemory.Address)
            )
        );
        ...
        emit BusinessRuleRequested(instanceId, BusinessRuleKey.Activity_0pzravo, requestId);
    }

    function Activity_0pzravo_Continue(uint256 instanceId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        BusinessRule storage br = inst.businessRules[BusinessRuleKey.Activity_0pzravo];
        require(br.exists, "business rule not set");
        ...
        inst.stateMemory.Distance = _extractJsonInt(raw, "distance");
        br.state = ElementState.COMPLETED;
        inst.gateways[GatewayKey.Gateway_0ckb278].state = ElementState.ENABLED;


        emit BusinessRuleCompleted(instanceId, BusinessRuleKey.Activity_0pzravo, br.requestId);
    }


function Event_17c6iyi(uint256 instanceId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        ActionEvent storage ev = inst.events[EventKey.Event_17c6iyi];
        ...
        emit ActionEventDone(instanceId, EventKey.Event_17c6iyi);
        inst.messages[MessageKey.Message_1wwub0e].state = ElementState.ENABLED;

    }

function Message_0gm7yco_Send(uint256 instanceId, string calldata fireflyTranId, string calldata PickUpTime) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.Message_0gm7yco];
        ...
        inst.stateMemory.PickUpTime = PickUpTime;
        m.state = ElementState.COMPLETED;
        emit MessageSent(instanceId, MessageKey.Message_0gm7yco, fireflyTranId);
        inst.messages[MessageKey.Message_10c1p7j].state = ElementState.DISABLED;
        inst.messages[MessageKey.Message_1l76ppu].state = ElementState.ENABLED;

    }

function Message_0nc7bj9_Send(uint256 instanceId, string calldata fireflyTranId, bool bikePrepare) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.Message_0nc7bj9];
        require(m.exists, "message not set");
        _checkParticipant(inst, m.sendParticipant);
        ...
        m.fireflyTranId = fireflyTranId;
        inst.stateMemory.BikePrepare = bikePrepare;
        m.state = ElementState.COMPLETED;
        emit MessageSent(instanceId, MessageKey.Message_0nc7bj9, fireflyTranId);
        inst.gateways[GatewayKey.Gateway_18xv5wc].state = ElementState.ENABLED;

    }

function Message_0phlgzx_Send(uint256 instanceId, string calldata fireflyTranId, string calldata DeliveryInfomation) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.Message_0phlgzx];
        require(m.exists, "message not set");
        _checkParticipant(inst, m.sendParticipant);
        ...
        m.fireflyTranId = fireflyTranId;
        inst.stateMemory.DeliveryInfomation = DeliveryInfomation;
        m.state = ElementState.COMPLETED;
        emit MessageSent(instanceId, MessageKey.Message_0phlgzx, fireflyTranId);
        inst.messages[MessageKey.Message_1hi03zn].state = ElementState.ENABLED;

    }

function Message_10c1p7j_Send(uint256 instanceId, string calldata fireflyTranId, string calldata Address) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.Message_10c1p7j];
        require(m.exists, "message not set");
        _checkParticipant(inst, m.sendParticipant);
        ...
        m.fireflyTranId = fireflyTranId;
        inst.stateMemory.Address = Address;
        m.state = ElementState.COMPLETED;
        emit MessageSent(instanceId, MessageKey.Message_10c1p7j, fireflyTranId);
        inst.messages[MessageKey.Message_0gm7yco].state = ElementState.DISABLED;
        inst.businessRules[BusinessRuleKey.Activity_0pzravo].state = ElementState.ENABLED;

    }

function Message_1hi03zn_Send(uint256 instanceId, string calldata fireflyTranId, string calldata Signature) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.Message_1hi03zn];
        require(m.exists, "message not set");
        _checkParticipant(inst, m.sendParticipant);
        ...
        m.fireflyTranId = fireflyTranId;
        inst.stateMemory.Signature = Signature;
        m.state = ElementState.COMPLETED;
        emit MessageSent(instanceId, MessageKey.Message_1hi03zn, fireflyTranId);
        inst.gateways[GatewayKey.Gateway_1rwnujh].state = ElementState.ENABLED;

    }

function Message_1kyybzd_Send(uint256 instanceId, string calldata fireflyTranId, bool carPrepare) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.Message_1kyybzd];
        require(m.exists, "message not set");
        _checkParticipant(inst, m.sendParticipant);
        ...
        m.fireflyTranId = fireflyTranId;
        inst.stateMemory.CarPrepare = carPrepare;
        m.state = ElementState.COMPLETED;
        emit MessageSent(instanceId, MessageKey.Message_1kyybzd, fireflyTranId);
        inst.gateways[GatewayKey.Gateway_18xv5wc].state = ElementState.ENABLED;

    }

function Message_1l76ppu_Send(uint256 instanceId, string calldata fireflyTranId, string calldata Signature) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.Message_1l76ppu];
        require(m.exists, "message not set");
        _checkParticipant(inst, m.sendParticipant);
        ...
        inst.stateMemory.Signature = Signature;
        m.state = ElementState.COMPLETED;
        emit MessageSent(instanceId, MessageKey.Message_1l76ppu, fireflyTranId);
        inst.gateways[GatewayKey.Gateway_1rwnujh].state = ElementState.ENABLED;

    }

function Message_1wwub0e_Send(uint256 instanceId, string calldata fireflyTranId, string calldata OrderContent) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.Message_1wwub0e];
        require(m.exists, "message not set");
        _checkParticipant(inst, m.sendParticipant);
        ...
        m.state = ElementState.COMPLETED;
        emit MessageSent(instanceId, MessageKey.Message_1wwub0e, fireflyTranId);
        inst.gateways[GatewayKey.Gateway_1k9qf30].state = ElementState.ENABLED;

    }

function Gateway_0ckb278(uint256 instanceId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Gateway storage g = inst.gateways[GatewayKey.Gateway_0ckb278];
        ...
        emit GatewayDone(instanceId, GatewayKey.Gateway_0ckb278);
        if (inst.stateMemory.Distance >= 10) {
            inst.messages[MessageKey.Message_0nc7bj9].state = ElementState.ENABLED;
        }
        else if (inst.stateMemory.Distance < 10) {
            inst.messages[MessageKey.Message_1kyybzd].state = ElementState.ENABLED;
        }

    }

function Gateway_18xv5wc(uint256 instanceId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Gateway storage g = inst.gateways[GatewayKey.Gateway_18xv5wc];
        require(g.exists, "gateway not set");
        require(g.state == ElementState.ENABLED, "gateway state not allowed");
        ...
        g.state = ElementState.COMPLETED;
        emit GatewayDone(instanceId, GatewayKey.Gateway_18xv5wc);
        inst.messages[MessageKey.Message_0phlgzx].state = ElementState.ENABLED;

    }

function Gateway_1rwnujh(uint256 instanceId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Gateway storage g = inst.gateways[GatewayKey.Gateway_1rwnujh];
        require(g.exists, "gateway not set");
        require(g.state == ElementState.ENABLED, "gateway state not allowed");
        ...
        g.state = ElementState.COMPLETED;
        emit GatewayDone(instanceId, GatewayKey.Gateway_1rwnujh);
        inst.events[EventKey.Event_1bxai2s].state = ElementState.ENABLED;

    }

function Gateway_1k9qf30(uint256 instanceId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Gateway storage g = inst.gateways[GatewayKey.Gateway_1k9qf30];
        require(g.exists, "gateway not set");
        require(g.state == ElementState.ENABLED, "gateway state not allowed");
        ...
        g.state = ElementState.COMPLETED;
        emit GatewayDone(instanceId, GatewayKey.Gateway_1k9qf30);
        inst.messages[MessageKey.Message_10c1p7j].state = ElementState.ENABLED;
        inst.messages[MessageKey.Message_0gm7yco].state = ElementState.ENABLED;

    }

function Event_1bxai2s(uint256 instanceId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        ActionEvent storage ev = inst.events[EventKey.Event_1bxai2s];
        require(ev.exists, "event not set");
        require(ev.state == ElementState.ENABLED, "event state not allowed");
        ...
        ev.state = ElementState.COMPLETED;
        emit ActionEventDone(instanceId, EventKey.Event_1bxai2s);
