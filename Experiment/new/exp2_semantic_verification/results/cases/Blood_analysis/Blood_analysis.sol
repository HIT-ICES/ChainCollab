...
contract Blood_analysis {
    ...
    struct StateMemory {
        bool FurtherCheck;
        string FurtherCheckDate;
        string FurtherCheckPlace;
        string PatientData;
    ...
    struct InitParameters {
        ...
        BusinessRuleInit Activity_1mj4mr7;
    }
    ...
    function createInstance(
        ...
        _createGateway(
            inst,
            GatewayKey.Gateway_1m6dgym,
            ElementState.DISABLED
        );

        _createBusinessRule(
            inst,
            BusinessRuleKey.Activity_1mj4mr7,
            params.Activity_1mj4mr7
        );
        ...
        )
    {
        ...
        {
            Message storage m = inst.messages[MessageKey.Message_0gd0z61];
        ...
        {
            Message storage m = inst.messages[MessageKey.Message_13mh6mk];
        ...
        {
            Gateway storage g = inst.gateways[GatewayKey.Gateway_0o8snyv];
        ...
        {
            Gateway storage g = inst.gateways[GatewayKey.Gateway_1m6dgym];
        ...
        {
            BusinessRule storage br = inst.businessRules[BusinessRuleKey.Activity_1mj4mr7];
            businessRuleStates[0] = uint8(br.state);
            businessRuleRequestIds[0] = br.requestId;
        }
    }

    function Activity_1mj4mr7(uint256 instanceId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        BusinessRule storage br = inst.businessRules[BusinessRuleKey.Activity_1mj4mr7];
        ...
        inputBody = _jsonAppendField(
            ...
            _jsonField(
                "result",
            _jsonString(inst.stateMemory.Result)
            )
        );
        ...
        emit BusinessRuleRequested(instanceId, BusinessRuleKey.Activity_1mj4mr7, requestId);
    }

    function Activity_1mj4mr7_Continue(uint256 instanceId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        BusinessRule storage br = inst.businessRules[BusinessRuleKey.Activity_1mj4mr7];
        require(br.exists, "business rule not set");
        ...
        inst.stateMemory.FurtherCheck = _extractJsonBool(raw, "FurtherCheck");
        br.state = ElementState.COMPLETED;
        inst.gateways[GatewayKey.Gateway_0o8snyv].state = ElementState.ENABLED;


        emit BusinessRuleCompleted(instanceId, BusinessRuleKey.Activity_1mj4mr7, br.requestId);
    }
...
function Message_0gd0z61_Send(uint256 instanceId, string calldata fireflyTranId, string calldata furtherCheckPlace, string calldata furtherCheckDate) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.Message_0gd0z61];
        ...
        inst.stateMemory.FurtherCheckPlace = furtherCheckPlace;
        inst.stateMemory.FurtherCheckDate = furtherCheckDate;
        m.state = ElementState.COMPLETED;
        emit MessageSent(instanceId, MessageKey.Message_0gd0z61, fireflyTranId);
        inst.events[EventKey.EndEvent_110myff].state = ElementState.ENABLED;

    }

function Message_0gswvmq_Send(uint256 instanceId, string calldata fireflyTranId, string calldata sample) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.Message_0gswvmq];
        require(m.exists, "message not set");
        _checkParticipant(inst, m.sendParticipant);
        ...
        m.fireflyTranId = fireflyTranId;
        inst.stateMemory.Sample = sample;
        m.state = ElementState.COMPLETED;
        emit MessageSent(instanceId, MessageKey.Message_0gswvmq, fireflyTranId);
        inst.messages[MessageKey.Message_1vzqd37].state = ElementState.ENABLED;

    }

function Message_0wq8mc6_Send(uint256 instanceId, string calldata fireflyTranId, string calldata patientData) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.Message_0wq8mc6];
        require(m.exists, "message not set");
        _checkParticipant(inst, m.sendParticipant);
        ...
        m.fireflyTranId = fireflyTranId;
        inst.stateMemory.PatientData = patientData;
        m.state = ElementState.COMPLETED;
        emit MessageSent(instanceId, MessageKey.Message_0wq8mc6, fireflyTranId);
        inst.messages[MessageKey.Message_13mh6mk].state = ElementState.ENABLED;

    }

function Message_13mh6mk_Send(uint256 instanceId, string calldata fireflyTranId, string calldata reportContent) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.Message_13mh6mk];
        require(m.exists, "message not set");
        _checkParticipant(inst, m.sendParticipant);
        ...
        m.fireflyTranId = fireflyTranId;
        inst.stateMemory.ReportContent = reportContent;
        m.state = ElementState.COMPLETED;
        emit MessageSent(instanceId, MessageKey.Message_13mh6mk, fireflyTranId);
        inst.gateways[GatewayKey.Gateway_1m6dgym].state = ElementState.ENABLED;

    }

function Message_1rqbibd_Send(uint256 instanceId, string calldata fireflyTranId, string calldata result) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.Message_1rqbibd];
        require(m.exists, "message not set");
        _checkParticipant(inst, m.sendParticipant);
        ...
        inst.businessRules[BusinessRuleKey.Activity_1mj4mr7].state = ElementState.ENABLED;

    }

function Message_1vzqd37_Send(uint256 instanceId, string calldata fireflyTranId, string calldata result) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.Message_1vzqd37];
        require(m.exists, "message not set");
        _checkParticipant(inst, m.sendParticipant);
...
function Gateway_0o8snyv(uint256 instanceId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Gateway storage g = inst.gateways[GatewayKey.Gateway_0o8snyv];
        ...
        emit GatewayDone(instanceId, GatewayKey.Gateway_0o8snyv);
        if (inst.stateMemory.FurtherCheck == false) {
            inst.events[EventKey.EndEvent_110myff].state = ElementState.ENABLED;
        }
        else if (inst.stateMemory.FurtherCheck == true) {
            inst.messages[MessageKey.Message_0gd0z61].state = ElementState.ENABLED;
        }
...
function Gateway_1m6dgym(uint256 instanceId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Gateway storage g = inst.gateways[GatewayKey.Gateway_1m6dgym];
        require(g.exists, "gateway not set");
        require(g.state == ElementState.ENABLED, "gateway state not allowed");
        ...
        g.state = ElementState.COMPLETED;
        emit GatewayDone(instanceId, GatewayKey.Gateway_1m6dgym);
        if (!(inst.messages[MessageKey.Message_1vzqd37].state == ElementState.COMPLETED && inst.messages[MessageKey.Message_13mh6mk].state == ElementState.COMPLETED)) {
