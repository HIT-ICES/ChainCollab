...
contract SetDisableCase {
    ...
    function createInstance(
        ...
        _createParticipant(
            inst,
            ParticipantKey.Seller,
            params.Seller_account,
            params.Seller_org,
            false,
            0,
            0
...
function Offer_Send(uint256 instanceId, string calldata fireflyTranId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.Offer];
...
function Cancel_Send(uint256 instanceId, string calldata fireflyTranId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.Cancel];
        require(m.exists, "message not set");
        _checkParticipant(inst, m.sendParticipant);
...
function Accept_Send(uint256 instanceId, string calldata fireflyTranId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.Accept];
        require(m.exists, "message not set");
        _checkParticipant(inst, m.sendParticipant);
...
function Decide(uint256 instanceId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Gateway storage g = inst.gateways[GatewayKey.Decide];
