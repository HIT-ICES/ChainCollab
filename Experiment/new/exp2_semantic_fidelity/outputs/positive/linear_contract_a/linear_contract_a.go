...
type StateMemory struct {
	ApprovedA bool `json:"ApprovedA"`
}

type InitParameters struct {
	SellerA Participant `json:"SellerA"`
	BuyerA Participant `json:"BuyerA"`
}
...
func (cc *SmartContract) CreateInstance(ctx contractapi.TransactionContextInterface, initParametersBytes string) (string, error) {
	...
	cc.CreateParticipant(ctx, &instance, "SellerA", initParameters.SellerA.MSP, initParameters.SellerA.Attributes, initParameters.SellerA.X509, initParameters.SellerA.IsMulti, 0, 0)
	cc.CreateParticipant(ctx, &instance, "BuyerA", initParameters.BuyerA.MSP, initParameters.BuyerA.Attributes, initParameters.BuyerA.X509, initParameters.BuyerA.IsMulti, 0, 0)
	cc.CreateActionEvent(ctx, &instance, "StartEvent_A", ENABLED)
	cc.CreateActionEvent(ctx, &instance, "EndEvent_A", DISABLED)
	cc.CreateMessage(ctx, &instance, "RequestA", "BuyerA", "SellerA", "", DISABLED, `{"schema": "{}"}`)
	cc.CreateMessage(ctx, &instance, "ConfirmA", "SellerA", "BuyerA", "", DISABLED, `{"schema": "{}"}`)
	cc.CreateGateway(ctx, &instance, "DecisionA", DISABLED)
...
func (cc *SmartContract) StartEvent_A(ctx contractapi.TransactionContextInterface, instanceID string) error {
	stub := ctx.GetStub()
	instance, err := cc.GetInstance(ctx, instanceID)

	actionEvent, err := cc.ReadEvent(ctx, instanceID, "StartEvent_A")
	if err != nil {
		return err
	}
	...
	cc.ChangeEventState(ctx, instance, "StartEvent_A", COMPLETED)
	stub.SetEvent("StartEvent_A", []byte("Contract has been started successfully"))
	cc.SetInstance(ctx, instance)
	cc.ChangeMsgState(ctx, instance, "RequestA", ENABLED)

	cc.SetInstance(ctx, instance)
	return nil
}

func (cc *SmartContract) RequestA_Send(ctx contractapi.TransactionContextInterface, instanceID string, fireflyTranID string) error {
	stub := ctx.GetStub()
	instance, err := cc.GetInstance(ctx, instanceID)
	msg, err := cc.ReadMsg(ctx, instanceID, "RequestA")
	if err != nil {
		return err
	}
	...
	cc.ChangeMsgState(ctx, instance, "RequestA", COMPLETED)

	stub.SetEvent("RequestA", []byte("Message is waiting for confirmation"))
	cc.SetInstance(ctx, instance)
	cc.SetInstance(ctx, instance)
	return nil
}

func (cc *SmartContract) RequestA_Complete(ctx contractapi.TransactionContextInterface, instanceID string) error {
	stub := ctx.GetStub()
	instance, err := cc.GetInstance(ctx, instanceID)
	msg, err := cc.ReadMsg(ctx, instanceID, "RequestA")
	if err != nil {
	...
	stub.SetEvent("RequestA", []byte("Message has been done"))
	cc.SetInstance(ctx, instance)
	cc.ChangeGtwState(ctx, instance, "DecisionA", ENABLED)

	cc.SetInstance(ctx, instance)
	return nil
}

func (cc *SmartContract) ConfirmA_Send(ctx contractapi.TransactionContextInterface, instanceID string, fireflyTranID string) error {
	stub := ctx.GetStub()
	instance, err := cc.GetInstance(ctx, instanceID)
	msg, err := cc.ReadMsg(ctx, instanceID, "ConfirmA")
	if err != nil {
		return err
	}
	...
	cc.ChangeMsgFireflyTranID(ctx, instance, fireflyTranID, msg.MessageID)
	cc.ChangeMsgState(ctx, instance, "ConfirmA", COMPLETED)

	stub.SetEvent("ConfirmA", []byte("Message is waiting for confirmation"))
	cc.SetInstance(ctx, instance)
	cc.SetInstance(ctx, instance)
	return nil
}

func (cc *SmartContract) ConfirmA_Complete(ctx contractapi.TransactionContextInterface, instanceID string) error {
	stub := ctx.GetStub()
	instance, err := cc.GetInstance(ctx, instanceID)
	msg, err := cc.ReadMsg(ctx, instanceID, "ConfirmA")
	if err != nil {
	...
	cc.ChangeMsgState(ctx, instance, msg.MessageID, COMPLETED)
	stub.SetEvent("ConfirmA", []byte("Message has been done"))
	cc.SetInstance(ctx, instance)
	cc.ChangeEventState(ctx, instance, "EndEvent_A", ENABLED)

	cc.SetInstance(ctx, instance)
	return nil
}

func (cc *SmartContract) DecisionA(ctx contractapi.TransactionContextInterface, instanceID string) error {
	stub := ctx.GetStub()
	instance, err := cc.GetInstance(ctx, instanceID)
	gtw, err := cc.ReadGtw(ctx, instanceID, "DecisionA")
	if err != nil {
		return err
	}
	...
	stub.SetEvent("DecisionA", []byte("Gateway has been completed"))
	cc.SetInstance(ctx, instance)
	if instance.InstanceStateMemory.ApprovedA == true {
		cc.ChangeMsgState(ctx, instance, "ConfirmA", ENABLED)
	} else {
		cc.ChangeEventState(ctx, instance, "EndEvent_A", ENABLED)
	}
...
func (cc *SmartContract) EndEvent_A(ctx contractapi.TransactionContextInterface, instanceID string) error {
	stub := ctx.GetStub()
	instance, err := cc.GetInstance(ctx, instanceID)
	actionEvent, err := cc.ReadEvent(ctx, instanceID, "EndEvent_A")
	if err != nil {
		return err
	}
	...
	cc.ChangeEventState(ctx, instance, "EndEvent_A", COMPLETED)
	stub.SetEvent("EndEvent_A", []byte("Event has been completed"))
	cc.SetInstance(ctx, instance)
	cc.SetInstance(ctx, instance)
	return nil
}
