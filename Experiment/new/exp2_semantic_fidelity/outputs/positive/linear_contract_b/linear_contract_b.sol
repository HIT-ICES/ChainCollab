...
contract LinearContractB {
    ...
    struct InitParameters {
        ...
        string dmnEvalUrl;
        bool enforceBusinessRuleCaller;
        address ProviderB_account;
        string ProviderB_org;
        address ClientB_account;
        string ClientB_org;
    }
    ...
    function createInstance(
        ...
        _createParticipant(
            inst,
            ParticipantKey.ProviderB,
            params.ProviderB_account,
            params.ProviderB_org,
            false,
            0,
            0
        );
        _createParticipant(
            inst,
            ParticipantKey.ClientB,
            params.ClientB_account,
            params.ClientB_org,
            false,
            0,
            0
...
function StartEvent_B(uint256 instanceId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        ActionEvent storage ev = inst.events[EventKey.StartEvent_B];
...
function QuoteB_Send(uint256 instanceId, string calldata fireflyTranId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.QuoteB];
...
function AckB_Send(uint256 instanceId, string calldata fireflyTranId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.AckB];
        require(m.exists, "message not set");
        _checkParticipant(inst, m.sendParticipant);
...
function ChoiceB(uint256 instanceId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Gateway storage g = inst.gateways[GatewayKey.ChoiceB];
...
function EndEvent_B(uint256 instanceId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        ActionEvent storage ev = inst.events[EventKey.EndEvent_B];
        require(ev.exists, "event not set");
        require(ev.state == ElementState.ENABLED, "event state not allowed");
