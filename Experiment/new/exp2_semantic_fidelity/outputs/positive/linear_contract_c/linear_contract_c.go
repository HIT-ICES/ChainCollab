...
type StateMemory struct {
	ReadyC bool `json:"ReadyC"`
}

type InitParameters struct {
	ManagerC Participant `json:"ManagerC"`
	WorkerC Participant `json:"WorkerC"`
}
...
func (cc *SmartContract) CreateInstance(ctx contractapi.TransactionContextInterface, initParametersBytes string) (string, error) {
	...
	cc.CreateParticipant(ctx, &instance, "ManagerC", initParameters.ManagerC.MSP, initParameters.ManagerC.Attributes, initParameters.ManagerC.X509, initParameters.ManagerC.IsMulti, 0, 0)
	cc.CreateParticipant(ctx, &instance, "WorkerC", initParameters.WorkerC.MSP, initParameters.WorkerC.Attributes, initParameters.WorkerC.X509, initParameters.WorkerC.IsMulti, 0, 0)
	cc.CreateActionEvent(ctx, &instance, "StartEvent_C", ENABLED)
	cc.CreateActionEvent(ctx, &instance, "EndEvent_C", DISABLED)
	cc.CreateMessage(ctx, &instance, "TaskC", "ManagerC", "WorkerC", "", DISABLED, `{"schema": "{}"}`)
	cc.CreateMessage(ctx, &instance, "ReportC", "WorkerC", "ManagerC", "", DISABLED, `{"schema": "{}"}`)
	cc.CreateGateway(ctx, &instance, "ReviewC", DISABLED)
...
func (cc *SmartContract) StartEvent_C(ctx contractapi.TransactionContextInterface, instanceID string) error {
	stub := ctx.GetStub()
	instance, err := cc.GetInstance(ctx, instanceID)

	actionEvent, err := cc.ReadEvent(ctx, instanceID, "StartEvent_C")
	if err != nil {
		return err
	}
	...
	cc.ChangeEventState(ctx, instance, "StartEvent_C", COMPLETED)
	stub.SetEvent("StartEvent_C", []byte("Contract has been started successfully"))
	cc.SetInstance(ctx, instance)
	cc.ChangeMsgState(ctx, instance, "TaskC", ENABLED)

	cc.SetInstance(ctx, instance)
	return nil
}

func (cc *SmartContract) TaskC_Send(ctx contractapi.TransactionContextInterface, instanceID string, fireflyTranID string) error {
	stub := ctx.GetStub()
	instance, err := cc.GetInstance(ctx, instanceID)
	msg, err := cc.ReadMsg(ctx, instanceID, "TaskC")
	if err != nil {
		return err
	}
	...
	cc.ChangeMsgState(ctx, instance, "TaskC", COMPLETED)

	stub.SetEvent("TaskC", []byte("Message is waiting for confirmation"))
	cc.SetInstance(ctx, instance)
	cc.SetInstance(ctx, instance)
	return nil
}

func (cc *SmartContract) TaskC_Complete(ctx contractapi.TransactionContextInterface, instanceID string) error {
	stub := ctx.GetStub()
	instance, err := cc.GetInstance(ctx, instanceID)
	msg, err := cc.ReadMsg(ctx, instanceID, "TaskC")
	if err != nil {
	...
	stub.SetEvent("TaskC", []byte("Message has been done"))
	cc.SetInstance(ctx, instance)
	cc.ChangeGtwState(ctx, instance, "ReviewC", ENABLED)

	cc.SetInstance(ctx, instance)
	return nil
}

func (cc *SmartContract) ReportC_Send(ctx contractapi.TransactionContextInterface, instanceID string, fireflyTranID string) error {
	stub := ctx.GetStub()
	instance, err := cc.GetInstance(ctx, instanceID)
	msg, err := cc.ReadMsg(ctx, instanceID, "ReportC")
	if err != nil {
		return err
	}
	...
	cc.ChangeMsgFireflyTranID(ctx, instance, fireflyTranID, msg.MessageID)
	cc.ChangeMsgState(ctx, instance, "ReportC", COMPLETED)

	stub.SetEvent("ReportC", []byte("Message is waiting for confirmation"))
	cc.SetInstance(ctx, instance)
	cc.SetInstance(ctx, instance)
	return nil
}

func (cc *SmartContract) ReportC_Complete(ctx contractapi.TransactionContextInterface, instanceID string) error {
	stub := ctx.GetStub()
	instance, err := cc.GetInstance(ctx, instanceID)
	msg, err := cc.ReadMsg(ctx, instanceID, "ReportC")
	if err != nil {
	...
	cc.ChangeMsgState(ctx, instance, msg.MessageID, COMPLETED)
	stub.SetEvent("ReportC", []byte("Message has been done"))
	cc.SetInstance(ctx, instance)
	cc.ChangeEventState(ctx, instance, "EndEvent_C", ENABLED)

	cc.SetInstance(ctx, instance)
	return nil
}

func (cc *SmartContract) ReviewC(ctx contractapi.TransactionContextInterface, instanceID string) error {
	stub := ctx.GetStub()
	instance, err := cc.GetInstance(ctx, instanceID)
	gtw, err := cc.ReadGtw(ctx, instanceID, "ReviewC")
	if err != nil {
		return err
	}
	...
	stub.SetEvent("ReviewC", []byte("Gateway has been completed"))
	cc.SetInstance(ctx, instance)
	if instance.InstanceStateMemory.ReadyC == true {
		cc.ChangeMsgState(ctx, instance, "ReportC", ENABLED)
	} else {
		cc.ChangeEventState(ctx, instance, "EndEvent_C", ENABLED)
	}
...
func (cc *SmartContract) EndEvent_C(ctx contractapi.TransactionContextInterface, instanceID string) error {
	stub := ctx.GetStub()
	instance, err := cc.GetInstance(ctx, instanceID)
	actionEvent, err := cc.ReadEvent(ctx, instanceID, "EndEvent_C")
	if err != nil {
		return err
	}
	...
	cc.ChangeEventState(ctx, instance, "EndEvent_C", COMPLETED)
	stub.SetEvent("EndEvent_C", []byte("Event has been completed"))
	cc.SetInstance(ctx, instance)
	cc.SetInstance(ctx, instance)
	return nil
}
