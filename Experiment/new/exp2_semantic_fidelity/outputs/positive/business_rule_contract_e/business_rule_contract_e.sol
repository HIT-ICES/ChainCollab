...
contract BusinessRuleContractE {
    ...
    struct InitParameters {
        ...
        string dmnEvalUrl;
        bool enforceBusinessRuleCaller;
        address TeamE_account;
        string TeamE_org;
        address LeadE_account;
        string LeadE_org;
        BusinessRuleInit RuleE;
    }
    ...
    function createInstance(
        ...
        _createParticipant(
            inst,
            ParticipantKey.TeamE,
            params.TeamE_account,
            params.TeamE_org,
            false,
            0,
            0
        ...
        _createParticipant(
            inst,
            ParticipantKey.LeadE,
            params.LeadE_account,
            params.LeadE_org,
            false,
            0,
            0
        ...
        _createActionEvent(
            inst,
            EventKey.StartEvent_BRE,
            ElementState.ENABLED
        );
        _createActionEvent(
            inst,
            EventKey.EndEvent_BRE,
    ...
    function RuleE(uint256 instanceId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        BusinessRule storage br = inst.businessRules[BusinessRuleKey.RuleE];
    ...
    function RuleE_Continue(uint256 instanceId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        BusinessRule storage br = inst.businessRules[BusinessRuleKey.RuleE];
        require(br.exists, "business rule not set");
...
function StartEvent_BRE(uint256 instanceId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        ActionEvent storage ev = inst.events[EventKey.StartEvent_BRE];
...
function InputE_Send(uint256 instanceId, string calldata fireflyTranId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.InputE];
...
function OutputE_Send(uint256 instanceId, string calldata fireflyTranId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.OutputE];
        require(m.exists, "message not set");
        _checkParticipant(inst, m.sendParticipant);
...
function GateE2(uint256 instanceId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Gateway storage g = inst.gateways[GatewayKey.GateE2];
...
function EndEvent_BRE(uint256 instanceId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        ActionEvent storage ev = inst.events[EventKey.EndEvent_BRE];
        require(ev.exists, "event not set");
        require(ev.state == ElementState.ENABLED, "event state not allowed");
