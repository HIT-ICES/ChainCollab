...
func (cc *SmartContract) CreateInstance(ctx contractapi.TransactionContextInterface, initParametersBytes string) (string, error) {
	...
	cc.CreateActionEvent(ctx, &instance, "MidEvent_1", DISABLED)
	cc.CreateActionEvent(ctx, &instance, "EndEvent_1", DISABLED)
	cc.CreateMessage(ctx, &instance, "Ping", "A", "B", "", DISABLED, `{"schema": "{}"}`)
...
func (cc *SmartContract) StartEvent_1(ctx contractapi.TransactionContextInterface, instanceID string) error {
	...
	cc.ChangeMsgState(ctx, instance, "Ping", ENABLED)

	cc.SetInstance(ctx, instance)
	return nil
}

func (cc *SmartContract) Ping_Send(ctx contractapi.TransactionContextInterface, instanceID string, fireflyTranID string) error {
	stub := ctx.GetStub()
	instance, err := cc.GetInstance(ctx, instanceID)
	msg, err := cc.ReadMsg(ctx, instanceID, "Ping")
	if err != nil {
		return err
	}
	...
	cc.ChangeMsgState(ctx, instance, "Ping", COMPLETED)

	stub.SetEvent("Ping", []byte("Message is waiting for confirmation"))
	cc.SetInstance(ctx, instance)
	cc.SetInstance(ctx, instance)
	return nil
}

func (cc *SmartContract) Ping_Complete(ctx contractapi.TransactionContextInterface, instanceID string) error {
	stub := ctx.GetStub()
	instance, err := cc.GetInstance(ctx, instanceID)
	msg, err := cc.ReadMsg(ctx, instanceID, "Ping")
	if err != nil {
	...
	stub.SetEvent("Ping", []byte("Message has been done"))
	cc.SetInstance(ctx, instance)
	cc.ChangeEventState(ctx, instance, "MidEvent_1", ENABLED)

	cc.SetInstance(ctx, instance)
	return nil
}

func (cc *SmartContract) MidEvent_1(ctx contractapi.TransactionContextInterface, instanceID string) error {
	stub := ctx.GetStub()
	instance, err := cc.GetInstance(ctx, instanceID)
	actionEvent, err := cc.ReadEvent(ctx, instanceID, "MidEvent_1")
	if err != nil {
		return err
	}
	...
	cc.ChangeEventState(ctx, instance, "MidEvent_1", COMPLETED)
	stub.SetEvent("MidEvent_1", []byte("Event has been completed"))
	cc.SetInstance(ctx, instance)
