...
contract EventFlowCase {
    ...
    enum EventKey {
        StartEvent_1,        MidEvent_1,        EndEvent_1    }
        ...
        )
    {
        ...
        messageStates = new uint8[](1);
        messageFireflyTranIds = new string[](1);
        {
            Message storage m = inst.messages[MessageKey.Ping];
        ...
        {
            ActionEvent storage ev = inst.events[EventKey.MidEvent_1];
...
function StartEvent_1(uint256 instanceId) external onlyInitialized {
        ...
        inst.messages[MessageKey.Ping].state = ElementState.ENABLED;

    }

function Ping_Send(uint256 instanceId, string calldata fireflyTranId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.Ping];
        ...
        emit MessageSent(instanceId, MessageKey.Ping, fireflyTranId);
        inst.events[EventKey.MidEvent_1].state = ElementState.ENABLED;

    }

function MidEvent_1(uint256 instanceId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        ActionEvent storage ev = inst.events[EventKey.MidEvent_1];
        require(ev.exists, "event not set");
        require(ev.state == ElementState.ENABLED, "event state not allowed");
        ...
        ev.state = ElementState.COMPLETED;
        emit ActionEventDone(instanceId, EventKey.MidEvent_1);
