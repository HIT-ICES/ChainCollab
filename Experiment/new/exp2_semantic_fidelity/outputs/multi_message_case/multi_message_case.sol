...
contract MultiMessageCase {
        ...
        )
    {
        ...
        {
            Gateway storage g = inst.gateways[GatewayKey.G1];
...
function StartEvent_1(uint256 instanceId) external onlyInitialized {
        ...
        inst.messages[MessageKey.M1].state = ElementState.ENABLED;

    }

function M1_Send(uint256 instanceId, string calldata fireflyTranId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.M1];
        ...
        emit MessageSent(instanceId, MessageKey.M1, fireflyTranId);
        inst.messages[MessageKey.M2].state = ElementState.ENABLED;

    }

function M2_Send(uint256 instanceId, string calldata fireflyTranId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.M2];
        require(m.exists, "message not set");
        _checkParticipant(inst, m.sendParticipant);
        ...
        m.fireflyTranId = fireflyTranId;
        m.state = ElementState.COMPLETED;
        emit MessageSent(instanceId, MessageKey.M2, fireflyTranId);
        inst.messages[MessageKey.M3].state = ElementState.ENABLED;

    }

function M3_Send(uint256 instanceId, string calldata fireflyTranId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Message storage m = inst.messages[MessageKey.M3];
        require(m.exists, "message not set");
        _checkParticipant(inst, m.sendParticipant);
        ...
        m.fireflyTranId = fireflyTranId;
        m.state = ElementState.COMPLETED;
        emit MessageSent(instanceId, MessageKey.M3, fireflyTranId);
        inst.events[EventKey.EndEvent_1].state = ElementState.ENABLED;

    }

function G1(uint256 instanceId) external onlyInitialized {
        Instance storage inst = _getInstance(instanceId);
        Gateway storage g = inst.gateways[GatewayKey.G1];
        ...
        emit GatewayDone(instanceId, GatewayKey.G1);
