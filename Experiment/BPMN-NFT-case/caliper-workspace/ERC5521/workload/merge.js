...
// -------------------- token2: batch mint WITH references to token1 (batch a) --------------------
class Token2RefMintWorkload_A extends WorkloadModuleBase {
  async initializeWorkloadModule(workerIndex, totalWorkers, roundIndex, roundArguments, sutAdapter, sutContext) {
    ...
    this.token1Path = path.join(this.poolDir, 'token1_ids_a.txt');
    this.token1ReadyPath = path.join(this.poolDir, 'token1_ids_a.ready');
    ...
    if (this.token1List.length === 0) {
      throw new Error(`[TOKEN2-REFMINT-A] token1 list is empty: ${this.token1Path}`);
    }
    ...
    console.log('[TOKEN2-REFMINT-A]', __filename, 'runId=', this.runId, 'token2Asked=', asked, 'limit=', this.limit, 'token1=', this.token1List.length, 'wrap=', this.wrap);

    // token2 manifest (optional, for later rounds)
    this.token2Path = path.join(this.poolDir, 'token2_ids_a.txt');
    this.token2LockPath = path.join(this.poolDir, 'token2_ids_a.lock');
    this.token2ReadyPath = path.join(this.poolDir, 'token2_ids_a.ready');
    initManifestOnce(this.token2Path, this.token2ReadyPath, path.join(this.poolDir, 'token2_ids_a.init.lock'));

    // Counter for token2 mint
    this.counterPath = path.join(this.poolDir, 'counter_token2_a.txt');
    this.lockPath = path.join(this.poolDir, 'counter_token2_a.lock');
    this.readyPath = path.join(this.poolDir, 'counter_token2_a.ready');
    resetCounterOnce(this.counterPath, this.readyPath, path.join(this.poolDir, 'counter_token2_a.init.lock'));
  ...
  async submitTransaction() {
    ...
    // token2 id
    const token2Id = `${this.runId}-token2-a-${idx}-5`;
...
module.exports.createWorkloadModule = () => new Token2RefMintWorkload_A();
