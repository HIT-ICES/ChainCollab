...
func (cc *SmartContract) CreateInstance(ctx contractapi.TransactionContextInterface, initParametersBytes string) (string, error) {
	...
	cc.CreateTokenElement(ctx, &instance, "Activity_0i3c0p3", DISABLED, `{"assetType":"transferable","operation":"mint","tokenName":"rawMaterial","caller":"Participant_0wg64sj","tokenType":"NFT","tokenId":"1","tokenURL":"http://www.github.com"}`, initParameters.ERCChaincodeNames["Activity_0i3c0p3"], initParameters.BpmnId)
	cc.CreateTokenElement(ctx, &instance, "Activity_11qv4ci", DISABLED, `{"assetType":"transferable","operation":"Transfer","tokenName":"rawMaterial","caller":"Participant_0wg64sj","callee":["Participant_18g06ah"],"tokenType":"NFT","tokenId":"1","tokenURL":"http://www.github.com"}`, initParameters.ERCChaincodeNames["Activity_11qv4ci"], initParameters.BpmnId)
	cc.CreateTokenElement(ctx, &instance, "Activity_12g7edm", DISABLED, `{"assetType":"transferable","operation":"query","tokenName":"rawMaterial","caller":"Participant_18g06ah","tokenType":"NFT","tokenId":"1","outputs":{"uriTest":{"type":"URI","dataType":"string"},"ownerTest":{"type":"owner","dataType":"string"}}}`, initParameters.ERCChaincodeNames["Activity_12g7edm"], initParameters.BpmnId)
	cc.CreateTokenElement(ctx, &instance, "Activity_0oqcguv", DISABLED, `{"assetType":"transferable","operation":"burn","tokenName":"rawMaterial","caller":"Participant_18g06ah","tokenType":"NFT","tokenId":"1"}`, initParameters.ERCChaincodeNames["Activity_0oqcguv"], initParameters.BpmnId)
...
func (cc *SmartContract) Activity_0i3c0p3_Continue(ctx contractapi.TransactionContextInterface, tokentestID string, instanceID string) error {
	...
	if err := json.Unmarshal(getpayload, &data); err != nil {
		return fmt.Errorf("failed to parse JSON: %v", err)
	}
	owner, _ := data["owner"].(string)
	if err := cc.changeTokenOwner(ctx, instance, tokenElement.TokenKey, owner); err != nil {
		return err
	}
	// 7. 改 TokenElement 状态 + 写回实例
...
func (cc *SmartContract) Activity_11qv4ci(ctx contractapi.TransactionContextInterface, from string, to string, tokentestID string, instanceID string) error {
	fmt.Println("---------------------------------")
	instance, err := cc.GetInstance(ctx, instanceID)
	if err != nil {
		return err
	...
	fmt.Println(callid)
	_args[2] = []byte(to)
	fmt.Println(calleeID[0]) // 这里假设只有一个接收者
	_args[3] = []byte(tokentestID)
...
func (cc *SmartContract) Message_1wyvezt_Send(ctx contractapi.TransactionContextInterface, instanceID string, fireflyTranID string) error {
	...
	cc.changeTokenElementState(ctx, instance, "Activity_0oqcguv", ENABLED)
	cc.SetInstance(ctx, instance)
	return nil
}
...
func (cc *SmartContract) Activity_0oqcguv(ctx contractapi.TransactionContextInterface, tokentestID string, instanceID string) error {
	fmt.Println("---------------------------------")
	instance, err := cc.GetInstance(ctx, instanceID)
	if err != nil {
		return err
	...
	//得到tokenElement元素
	tokenElement, err := cc.ReadTokenElement(ctx, instanceID, "Activity_0oqcguv")
	if err != nil {
		return err
	}
	//得到token元素
	...
	// // //改状态
	// // cc.changeTokenElementState(ctx, instance, "Activity_0oqcguv", COMPLETED)
	// cc.SetInstance(ctx, instance)
...
func (cc *SmartContract) Activity_query(ctx contractapi.TransactionContextInterface, tokentestID string, instanceID string) error {
	fmt.Println("---------------------------------")
	instance, err := cc.GetInstance(ctx, instanceID)
	if err != nil {
		return err
