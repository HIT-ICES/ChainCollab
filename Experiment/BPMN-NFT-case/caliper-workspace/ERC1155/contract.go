...
// getAndIncrementNextTokenID：保持签名不变，但不再读写 nextTokenIDKey（去热点）
func (s *SmartContract) getAndIncrementNextTokenID(ctx contractapi.TransactionContextInterface) (uint64, error) {
	txid := ctx.GetStub().GetTxID()
	if txid == "" {
		return 0, fmt.Errorf("TxID 为空，无法生成候选 ID")
	}
	return hash64("base|" + txid), nil
}

// 1) 先查 aliasKey，已存在则直接返回（幂等）
// 2) 不存在则用候选 id + 探测 salt，找到未占用的 idKey
// 3) 写入双向 KV：aliasKey->id、idKey->alias
func (s *SmartContract) RegisterTokenAlias(ctx contractapi.TransactionContextInterface, alias string, InstanceID string) (uint64, error) {
	...
	if alias == "" {
		return 0, fmt.Errorf("alias 不能为空")
	}

	aliasKey := aliasToIDPrefix + alias + "_" + InstanceID
	...
	// 2) 获取本交易的 baseSeed（不读写链上共享 key，因此无热点）
	baseSeed, err := s.getAndIncrementNextTokenID(ctx)
	if err != nil {
		return 0, err
	}

	// 3) 探测：避免极小概率的 uint64 碰撞（或历史遗留占用）
	const maxProbe = 2000
	for probe := 0; probe < maxProbe; probe++ {
		// 候选 ID：把 InstanceID + alias + baseSeed + probe 组合进 hash
		// - InstanceID 参与：避免不同实例的 alias 互相影响
		// - probe：若 idKey 被占用则换一个
		candidate := hash64(fmt.Sprintf("id|%s|%s|%d|%d", InstanceID, alias, baseSeed, probe))

		newIDString := strconv.FormatUint(candidate, 10)
		idKey := IDToAliasPrefix + newIDString

		occupiedBytes, err := ctx.GetStub().GetState(idKey)
		if err != nil {
			return 0, fmt.Errorf("查询数字 ID 键 '%s' 失败: %v", idKey, err)
		}

		if occupiedBytes == nil {
			// 找到空位：写入双向映射
		...
		// 如果这个 idKey 已经对应同一个 alias，也视为成功（可用于“修复”aliasKey缺失等极端情况）
		if string(occupiedBytes) == alias {
			if err := ctx.GetStub().PutState(aliasKey, []byte(newIDString)); err != nil {
				return 0, fmt.Errorf("写入别名到 ID 映射失败: %v", err)
			}
			return candidate, nil
	...
	return 0, fmt.Errorf("无法为 alias='%s' 分配唯一 uint64（探测次数超过 %d）", alias, maxProbe)
}
...
func (s *SmartContract) Getuint64IDByString(ctx contractapi.TransactionContextInterface, alias string, InstanceID string) (uint64, error) {
	key := aliasToIDPrefix + alias + "_" + InstanceID

	idBytes, err := ctx.GetStub().GetState(key)
	if err != nil {
		return 0, fmt.Errorf("查询别名键 '%s' 失败: %v", key, err)
	}
	if idBytes == nil {
		return 0, fmt.Errorf("别名 '%s' 尚未注册", alias)
	}
