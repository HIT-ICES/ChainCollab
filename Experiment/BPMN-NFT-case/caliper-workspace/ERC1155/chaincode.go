...
func (cc *SmartContract) CreateInstance(ctx contractapi.TransactionContextInterface, initParametersBytes string) (string, error) {
	...
	cc.CreateTokenElement(ctx, &instance, "Activity_0udosg1", DISABLED, `{"assetType":"distributive","operation":"mint","tokenName":"EBook","caller":"Participant_19kwe8n","tokenId":"1","tokenURL":"www.github.com"}`, initParameters.ERCChaincodeNames["Activity_0udosg1"], initParameters.BpmnId)
	cc.CreateTokenElement(ctx, &instance, "Activity_0ciiqug", DISABLED, `{"assetType":"distributive","operation":"grant usage rights","tokenName":"EBook","caller":"Participant_19kwe8n","callee":["Participant_0mcenv6"],"tokenId":"1"}`, initParameters.ERCChaincodeNames["Activity_0ciiqug"], initParameters.BpmnId)
	cc.CreateTokenElement(ctx, &instance, "Activity_013vhiy", DISABLED, `{"assetType":"distributive","operation":"transfer","tokenName":"EBook","caller":"Participant_19kwe8n","callee":["Participant_0mcenv6"],"tokenId":"1"}`, initParameters.ERCChaincodeNames["Activity_013vhiy"], initParameters.BpmnId)
	cc.CreateTokenElement(ctx, &instance, "Activity_0jgcjkn", DISABLED, `{"assetType":"distributive","operation":"burn","tokenName":"EBook","caller":"Participant_0mcenv6","tokenId":"1"}`, initParameters.ERCChaincodeNames["Activity_0jgcjkn"], initParameters.BpmnId)
	cc.CreateTokenElement(ctx, &instance, "Activity_14lrjch", DISABLED, `{"assetType":"distributive","operation":"revoke usage rights","tokenName":"EBook","caller":"Participant_19kwe8n","callee":["Participant_0mcenv6"],"tokenId":"1"}`, initParameters.ERCChaincodeNames["Activity_14lrjch"], initParameters.BpmnId)
	cc.CreateTokenElement(ctx, &instance, "Activity_0wq3tza", DISABLED, `{"assetType":"distributive","operation":"query","tokenName":"EBook","caller":"Participant_0mcenv6","tokenId":"1","outputs":{"isowner":{"type":"isowner","dataType":"boolean"},"isuser":{"type":"isuser","dataType":"boolean"},"uri":{"type":"URI","dataType":"string"}}}`, initParameters.ERCChaincodeNames["Activity_0wq3tza"], initParameters.BpmnId)
...
func (cc *SmartContract) Message_183xuj5_Send(ctx contractapi.TransactionContextInterface, instanceID string, fireflyTranID string) error {
	...
	cc.changeTokenElementState(ctx, instance, "Activity_0wq3tza", ENABLED)
	cc.SetInstance(ctx, instance)
	return nil
}
...
func (cc *SmartContract) Activity_0udosg1(ctx contractapi.TransactionContextInterface, instanceID string) error {
	fmt.Println("----- Activity_0udosg1 (Start) -----")
	...
	eventPayload := map[string]string{
		"InstanceID": instanceID,
		"ActivityID": "Activity_0udosg1",
		"Func":       "Activity_0udosg1_Continue",
	}
	eventPayloadAsBytes, err := json.Marshal(eventPayload)
	if err != nil {
		return fmt.Errorf("failed to marshal event payload: %v", err)
	...
	if err := cc.changeTokenElementState(ctx, instance, "Activity_0udosg1", WAITINGFORCONFIRMATION); err != nil {
		return err
	}
...
func (cc *SmartContract) Activity_0udosg1_Continue(ctx contractapi.TransactionContextInterface, testtokenID string, instanceID string) error {
	fmt.Println("----- Activity_0udosg1_Continue -----")
	...
	// 3.读 CID
	// cid, err := cc.GetURLData(ctx, instanceID, "Activity_0udosg1")
	// fmt.Println(cid)
	...
	_args[2] = []byte(testtokenID) //tokenid+后缀区分所有权还是
	_args[3] = []byte("50")
...
func (cc *SmartContract) Activity_013vhiy(ctx contractapi.TransactionContextInterface, testtokenID string, testtokenID2 string, instanceID string) error {
	instance, err := cc.GetInstance(ctx, instanceID)
	if err != nil {
		return err
	}
	...
	//得到tokenElement元素
	tokenElement, err := cc.ReadTokenElement(ctx, instanceID, "Activity_013vhiy")
	if err != nil {
		return err
	}
	//得到token元素
	...
	_args[1] = []byte(testtokenID)
	_args[2] = []byte(testtokenID2)
	_args[3] = []byte(testtokenID) //tokenid+后缀区分所有权还是
	_args[4] = []byte("5")
	...
	// //修改token对象的拥有者
	// cc.changeTokenOwner(ctx, instance, tokenElement.TokenKey, callee11)

	// //改状态
	// cc.changeTokenElementState(ctx, instance, "Activity_013vhiy", COMPLETED)
	// cc.SetInstance(ctx, instance)

	// cc.changeTokenElementState(ctx, instance, "Activity_0jgcjkn", ENABLED)
	// cc.SetInstance(ctx, instance)
	return nil
}

func (cc *SmartContract) Activity_0jgcjkn(ctx contractapi.TransactionContextInterface, testtokenID string, instanceID string) error {
	instance, err := cc.GetInstance(ctx, instanceID)
	if err != nil {
		return err
	}
	...
	//得到tokenElement元素
	tokenElement, err := cc.ReadTokenElement(ctx, instanceID, "Activity_0jgcjkn")
	if err != nil {
		return err
	}
	// //得到token元素
	...
	// 	fmt.Println("token.Flagdistributive为")
	// 	fmt.Println(token.Flagdistributive)
	// 	return fmt.Errorf("所有权没有创建，不能销毁")
	// }
	...
	// cc.changeTokenFlag(ctx, instance, tokenElement.TokenKey, false)

	// //改状态
	// cc.changeTokenElementState(ctx, instance, "Activity_0jgcjkn", COMPLETED)
	// cc.SetInstance(ctx, instance)
...
func (cc *SmartContract) Activity_14lrjch(ctx contractapi.TransactionContextInterface, instanceID string) error {
	instance, err := cc.GetInstance(ctx, instanceID)
	if err != nil {
		return err
	}
	...
	//得到tokenElement元素
	tokenElement, err := cc.ReadTokenElement(ctx, instanceID, "Activity_14lrjch")
	if err != nil {
		return err
	}
	//得到token元素
	...
	//所有权创建检查
	if token.Flagdistributive == false {
		return fmt.Errorf("所有权未创建，不能使用权")
	}

	//
	if token.OwnerID != callid {
		return fmt.Errorf("没有权限，不能撤销")
	}
	var calleeID []string
	//得到编码后的账户地址
	for _, callee := range tokenElement.CalleeID {
	...
	//改状态
	cc.changeTokenElementState(ctx, instance, "Activity_14lrjch", COMPLETED)
	cc.SetInstance(ctx, instance)

	cc.changeTokenElementState(ctx, instance, "Activity_013vhiy", ENABLED)
	cc.SetInstance(ctx, instance)
	return nil
}

func (cc *SmartContract) Activity_0wq3tza(ctx contractapi.TransactionContextInterface, testtokenID string, instanceID string) error {
	fmt.Println("----- Activity_0wq3tza -----")
	instance, err := cc.GetInstance(ctx, instanceID)
	if err != nil {
		return err
	}
	...
	tokenElement, err := cc.ReadTokenElement(ctx, instanceID, "Activity_0wq3tza")
	if err != nil {
		return err
	}
	...
	args := make([][]byte, 4)
	args[0] = []byte("QueryBalance")
	args[1] = []byte(testtokenID)
	args[2] = []byte(testtokenID)
	args[3] = []byte(instanceID)

	_, err = cc.Invoke_Other_chaincode(
		ctx,
		chaincodeName,
		"default",
		args,
	...
	// cc.changeTokenElementState(ctx, instance, "Activity_0wq3tza", COMPLETED)
	// cc.SetInstance(ctx, instance)

	// cc.changeTokenElementState(ctx, instance, "Activity_14lrjch", ENABLED)
	// cc.SetInstance(ctx, instance)
