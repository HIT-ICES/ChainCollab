...
class MintWorkload extends WorkloadModuleBase {
  async initializeWorkloadModule(workerIndex, totalWorkers, roundIndex, roundArguments, sutAdapter, sutContext) {
    ...
    this.limit = parseInt(this.roundArguments.mintCount, 10);
    this.mintStart = parseInt(this.roundArguments.mintStart || '0', 10);

    console.log('[MINT]', __filename, 'runId=', this.runId, 'limit=', this.limit);
    ...
    // Counter for mint
    this.counterPath = path.join(this.poolDir, 'counter_mint.txt');
    this.lockPath = path.join(this.poolDir, 'counter_mint.lock');
    this.readyPath = path.join(this.poolDir, 'counter_mint.ready');
    resetCounterOnce(this.counterPath, this.readyPath, path.join(this.poolDir, 'counter_mint.init.lock'));
  ...
  async submitTransaction() {
    ...
    const tokenId = `${this.runId}-mint-${this.mintStart + idx}-5`;

    await this.sutAdapter.sendRequests({
      channel,
      contractId,
      contractFunction: 'Activity_0udosg1_Continue',
