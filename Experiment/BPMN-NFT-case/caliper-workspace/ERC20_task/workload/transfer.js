...
class TransferWorkload extends WorkloadModuleBase {
  async initializeWorkloadModule(workerIndex, totalWorkers, roundIndex, roundArguments, sutAdapter, sutContext) {
    ...
    this.limit = parseInt(this.roundArguments.transferCount, 10);

    console.log('[TRANSFER]', __filename, 'runId=', this.runId, 'limit=', this.limit);

    this.poolDir = resolvePoolDir(this.runId, this.roundArguments.poolBaseDir);
    fs.mkdirSync(this.poolDir, { recursive: true });

    // Wait until manifest exists
    ...
    this.pool = all.filter(id => id.includes('-prepT-'));
    if (this.pool.length < this.limit) {
      throw new Error(`transfer pool not enough: need ${this.limit}, got ${this.pool.length}. (premint success?)`);
    }
    this.burnPool = all.filter(id => id.includes('-prepB-'));
    if (this.burnPool.length < this.limit) {
      throw new Error(`burn pool not enough for tokenId2: need ${this.limit}, got ${this.burnPool.length}. (premint prepBurnCount >= transferCount?)`);
    }
    // Counter for transfer consumption
    this.counterPath = path.join(this.poolDir, 'counter_transfer.txt');
    this.lockPath = path.join(this.poolDir, 'counter_transfer.lock');
    this.readyPath = path.join(this.poolDir, 'counter_transfer.ready');
    resetCounterOnce(this.counterPath, this.readyPath, path.join(this.poolDir, 'counter_transfer.init.lock'));

    this.from = this.roundArguments.from || 'x509::CN=user1,OU=client::CN=ca.mem.org.com,OU=Fabric,O=mem.org.com,ST=North Carolina,C=US';
    this.to = this.roundArguments.to || 'x509::CN=user2,OU=client::CN=ca.mem.org.com,OU=Fabric,O=mem.org.com,ST=North Carolina,C=US';
  }
  ...
  async submitTransaction() {
    ...
    const tokenId2 = this.burnPool[idx];
    ...
    await this.sutAdapter.sendRequests({
      channel,
      contractId,
      contractFunction: 'Activity_19nbvrt',
      contractArguments: [tokenId, tokenId2, "0"],
