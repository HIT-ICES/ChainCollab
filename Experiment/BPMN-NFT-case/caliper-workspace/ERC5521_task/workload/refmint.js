...
// -------------------- token2: batch mint WITH references to token1 --------------------
class Token2RefMintWorkload extends WorkloadModuleBase {
  async initializeWorkloadModule(workerIndex, totalWorkers, roundIndex, roundArguments, sutAdapter, sutContext) {
    ...
    this.contractFunction = 'Activity_0vjedml_Continue'; 
    ...
    if (this.token1List.length === 0) {
      throw new Error(`[TOKEN2-REFMINT] token1 list is empty: ${this.token1Path}`);
    }
    ...
    console.log('[TOKEN2-REFMINT]', __filename, 'runId=', this.runId, 'token2Asked=', asked, 'limit=', this.limit, 'token1=', this.token1List.length, 'wrap=', this.wrap);

    // token2 manifest (optional, for later rounds)
    this.token2Path = path.join(this.poolDir, 'token2_ids.txt');
    this.token2LockPath = path.join(this.poolDir, 'token2_ids.lock');
    this.token2ReadyPath = path.join(this.poolDir, 'token2_ids.ready');
    initManifestOnce(this.token2Path, this.token2ReadyPath, path.join(this.poolDir, 'token2_ids.init.lock'));

    // Counter for token2 mint
    this.counterPath = path.join(this.poolDir, 'counter_token2.txt');
    this.lockPath = path.join(this.poolDir, 'counter_token2.lock');
    this.readyPath = path.join(this.poolDir, 'counter_token2.ready');
    resetCounterOnce(this.counterPath, this.readyPath, path.join(this.poolDir, 'counter_token2.init.lock'));
  ...
  async submitTransaction() {
    ...
    // token2 id
    const token2Id = `${this.runId}-token2-${idx}-5`;
    ...
    await this.sutAdapter.sendRequests({
      ...
      contractArguments: [token2Id, tokenIdsJson,'0'],
...
module.exports.createWorkloadModule = () => new Token2RefMintWorkload();
