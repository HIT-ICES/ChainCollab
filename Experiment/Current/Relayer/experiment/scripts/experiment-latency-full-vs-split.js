...
function resolvePreferredFnsFromSplitPlan(splitCase, fullAbi) {
  ...
  const sourceAll = sourceNodeIds
    .map((id) => safeIdentifier(id))
    .filter((fn) => fullFnSet.has(fn));
  const targetAll = targetNodeIds
    .map((id) => safeIdentifier(id))
    .filter((fn) => fullFnSet.has(fn));

  const markerFn = marker ? safeIdentifier(marker) : "";
  const markerIdx = markerFn ? sourceAll.indexOf(markerFn) : -1;
  let sourcePre = [];
  let sourcePost = [];
  if (markerIdx >= 0) {
    sourcePre = sourceAll.slice(0, markerIdx + 1);
    sourcePost = sourceAll.slice(markerIdx + 1);
  } else {
    const half = Math.ceil(sourceAll.length / 2);
    sourcePre = sourceAll.slice(0, half);
    sourcePost = sourceAll.slice(half);
  }
  return [...sourcePre, ...targetAll, ...sourcePost];
}
...
function compileContractFromSource(solPath, contractName) {
  ...
  const targetName = contractName || Object.keys(contractsByFile)[0];
  const target = contractsByFile[targetName];
  if (!target) {
...
function buildParamValue(input, deployerAddress, identityAddress, instanceIdHint) {
  ...
  if (t === "string") {
    return n.toLowerCase().includes("org") ? "ORG" : "tx";
  }
...
async function runFullWorkflow({
  ...
  const endedOkByEndEvent = hasEndEvent
    ? executedNames.some((n) => String(n).startsWith("EndEvent_"))
    : false;
...
async function runFullOnChainBaseline({
  ...
  for (const splitCase of splitCaseSpec.cases || []) {
    ...
    const compiled = compileContractFromSource(solidityPath, "WorkflowContract");
    const preferredFns = resolvePreferredFnsFromSplitPlan(splitCase, compiled.abi);
