...
contract Blood_analysisSubmodelA {
    struct HandoffTask {
        address requester;
        bytes32 payloadHash;
        uint256 nonce;
    ...
    function _nodeOrder(string memory nodeId) internal pure returns (uint256) {
        if (keccak256(bytes(nodeId)) == keccak256(bytes("StartEvent_0m7hz56"))) return 0;
        if (keccak256(bytes(nodeId)) == keccak256(bytes("ParallelGateway_1pgjqtw"))) return 1;
        if (keccak256(bytes(nodeId)) == keccak256(bytes("ChoreographyTask_10lckh7"))) return 2;
        if (keccak256(bytes(nodeId)) == keccak256(bytes("ChoreographyTask_131v44n"))) return 3;
        if (keccak256(bytes(nodeId)) == keccak256(bytes("ChoreographyTask_1732eky"))) return 4;
        if (keccak256(bytes(nodeId)) == keccak256(bytes("ParallelGateway_16ab76f"))) return 5;
        if (keccak256(bytes(nodeId)) == keccak256(bytes("ChoreographyTask_1bkb191"))) return 6;
        if (keccak256(bytes(nodeId)) == keccak256(bytes("EndEvent_110myff"))) return 7;
        revert("unknown node id");
    }

    function StartEvent_0m7hz56(bytes32 taskId, bytes32 payloadHash) external {
        require(taskExists[taskId], "unknown taskId");
        _markNodeExecuted(taskId, "StartEvent_0m7hz56", payloadHash);
    }
    function ParallelGateway_1pgjqtw(bytes32 taskId, bytes32 payloadHash) external {
        require(taskExists[taskId], "unknown taskId");
        _markNodeExecuted(taskId, "ParallelGateway_1pgjqtw", payloadHash);
    }
    function ChoreographyTask_10lckh7(bytes32 taskId, bytes32 payloadHash) external {
        require(taskExists[taskId], "unknown taskId");
        _markNodeExecuted(taskId, "ChoreographyTask_10lckh7", payloadHash);
    }
    function ChoreographyTask_131v44n(bytes32 taskId, bytes32 payloadHash) external {
        require(taskExists[taskId], "unknown taskId");
        _markNodeExecuted(taskId, "ChoreographyTask_131v44n", payloadHash);
    }
    function ChoreographyTask_1732eky(bytes32 taskId, bytes32 payloadHash) external {
        require(taskExists[taskId], "unknown taskId");
        _markNodeExecuted(taskId, "ChoreographyTask_1732eky", payloadHash);
    }
    function ParallelGateway_16ab76f(bytes32 taskId, bytes32 payloadHash) external {
        require(taskExists[taskId], "unknown taskId");
        _markNodeExecuted(taskId, "ParallelGateway_16ab76f", payloadHash);
    }
    function ChoreographyTask_1bkb191(bytes32 taskId, bytes32 payloadHash) external {
        require(taskExists[taskId], "unknown taskId");
        _markNodeExecuted(taskId, "ChoreographyTask_1bkb191", payloadHash);
    }
    function EndEvent_110myff(bytes32 taskId, bytes32 payloadHash) external {
        require(taskExists[taskId], "unknown taskId");
        _markNodeExecuted(taskId, "EndEvent_110myff", payloadHash);
    }
}
