...
contract Coffee_machineSubmodelB {
    enum RelayStatus {
        Rejected,
        Pending,
        Accepted,
    ...
    function _nodeOrder(string memory nodeId) internal pure returns (uint256) {
        if (keccak256(bytes(nodeId)) == keccak256(bytes("AUTO_EXIT_A"))) return 0;
        revert("unknown node id");
    }

    function AUTO_EXIT_A(bytes32 taskId) external {
        require(deliveries[taskId].processed, "handoff not accepted");
        _markNodeExecuted(taskId, "AUTO_EXIT_A", deliveries[taskId].payloadHash);
    }
}
