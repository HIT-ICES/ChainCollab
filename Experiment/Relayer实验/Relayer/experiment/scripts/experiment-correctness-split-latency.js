  ...
  while (remaining.length && guard < 2000) {
    ...
    if (RELAYER_USE_BATCH && remaining.length > 1 && typeof contract.executeBatch === "function") {
      try {
        ...
        await waitForNextBlock(provider, rc.blockNumber, `${phase} case=${caseId} executeBatch`);
    ...
    for (let i = 0; i < remaining.length; i++) {
      ...
      try {
        ...
        const block = await provider.getBlock(rc.blockNumber);
        await waitForNextBlock(provider, rc.blockNumber, `${phase} case=${caseId} fn=${fn}`);
        if (!firstTs) {
  ...
  while (remaining.length && guard < 2000) {
    ...
    if (RELAYER_USE_BATCH && remaining.length > 1 && typeof contract.executeBatch === "function") {
      try {
        ...
        const block = await provider.getBlock(rc.blockNumber);
        await waitForNextBlock(provider, rc.blockNumber, `target case=${caseId} executeBatch`);
        const ts = Number(block?.timestamp || 0);
        if (!firstTs) {
          firstTs = ts;
        }
    ...
    for (let i = 0; i < remaining.length; i++) {
      ...
      try {
        ...
        const block = await provider.getBlock(rc.blockNumber);
        await waitForNextBlock(provider, rc.blockNumber, `target case=${caseId} fn=${fn}`);
        if (!firstTs) {
          firstTs = Number(block?.timestamp || 0);
        }
        lastTs = Number(block?.timestamp || 0);
...
async function main() {
  ...
  for (const caseId of deploymentByCase.keys()) {
    ...
    for (let i = 0; i < workflowRuns; i++) {
      ...
      await waitForNextBlock(sourceProvider, startRc.blockNumber, `start handoff case=${caseId} round=${i}`);
      ...
      if (inlineRelay) {
        ...
        await waitForNextBlock(targetProvider, relayRc.blockNumber, `inline relay case=${caseId} round=${i}`);
      ...
      await waitForNextBlock(targetProvider, relayRc.blockNumber, `relay case=${caseId} round=${i}`);
